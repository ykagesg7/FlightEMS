-- 航空工学 Mission E-5: エンジン理論と性能記事の登録
-- 2025-08-15 作成
...
INSERT INTO learning_contents (
    ...
    'CPL学科',
    '航空機エンジンの基本原理からジェットエンジン、アフターバーナーまでを徹底解説。レシプロエンジンの4サイクル、ジェットエンジンの連続燃焼サイクル、アフターバーナーの爆発的パワー増大を理解し、エンジン性能の本質を習得。',
    325,
    null,
    'article',
    true,
...
-- サブカテゴリ設定（タグ相当）
UPDATE learning_contents SET sub_category = '航空工学' WHERE id = '3.2.5_EngineTheory';

-- ===============================
-- 3. 関連する航空工学問題とのマッピング（将来的な拡張用）
-- ===============================

-- エンジン理論・性能関連の問題IDを配列で設定
-- 注意: 実際の統一CPL問題IDは実データに基づいて設定する必要があります
UPDATE learning_test_mapping
SET unified_cpl_question_ids = ARRAY[
    -- エンジン理論・性能関連の問題IDをここに配列で設定
    -- 例: (SELECT ARRAY_AGG(id) FROM unified_cpl_questions
    --      WHERE main_subject = '航空工学' AND sub_subject LIKE '%エンジン%' OR sub_subject LIKE '%ジェット%' LIMIT 10)
]::UUID[]
WHERE learning_content_id = '3.2.5_EngineTheory';
