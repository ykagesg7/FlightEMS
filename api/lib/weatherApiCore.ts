/**
 * WeatherAPI.com プロキシの共有ロジック（Vercel api ハンドラと Vite dev プラグイン）
 *
 * 実体は本ディレクトリに置く（`api/lib/*` は Vercel が CommonJS でバンドルする）。
 * プロジェクト直下の `lib/` に置くと ESM 出力となり `require` と整合せず本番で 500 になる。
 *
