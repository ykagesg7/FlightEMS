-- PPL Subject 4 Phase 1 第8本 mapping — 緊急・遭難 34問（20+9+3+1+1）
-- Clusters: 捜索救難業務/緊急機の行動, 捜索救難業務/救難の実施手続き,
--           緊急機に対する管制 (+ 下位2クラスタ。PPLタグ無しも同趣旨のため含む)
-- Apply: Supabase MCP execute_sql (project_id = fstynltdfdetpyvbrswr)

INSERT INTO learning_test_mapping (
  learning_content_id,
  content_title,
  content_category,
  test_question_ids,
  unified_cpl_question_ids,
  topic_category,
  subject_area,
  mapping_source,
  verification_status
)
SELECT
  lc.id,
  lc.title,
  lc.category,
  (SELECT COALESCE(array_agg(q.id::text ORDER BY q.id), ARRAY[]::text[])
   FROM unified_cpl_questions q
   WHERE q.verification_status = 'verified'
     AND q.main_subject = '航空通信'
     AND q.sub_subject = v.sub_subject
     AND (
       q.applicable_exams @> ARRAY['PPL']::text[]
       OR v.sub_subject LIKE '緊急機に対する管制/%'
     )),
  (SELECT COALESCE(array_agg(q.id ORDER BY q.id), ARRAY[]::uuid[])
   FROM unified_cpl_questions q
   WHERE q.verification_status = 'verified'
     AND q.main_subject = '航空通信'
     AND q.sub_subject = v.sub_subject
     AND (
       q.applicable_exams @> ARRAY['PPL']::text[]
       OR v.sub_subject LIKE '緊急機に対する管制/%'
     )),
  v.topic_category,
  '航空通信',
  'ppl432_20260722_mayday_pan_pan',
  'verified'
FROM learning_contents lc
JOIN (VALUES
  ('PPL-4-3-2_MaydayPanPanBasics', '捜索救難業務/緊急機の行動', '航空通信/緊急機の行動'),
  ('PPL-4-3-2_MaydayPanPanBasics', '捜索救難業務/救難の実施手続き', '航空通信/救難の実施手続き'),
  ('PPL-4-3-2_MaydayPanPanBasics', '緊急機に対する管制', '航空通信/緊急機に対する管制'),
  ('PPL-4-3-2_MaydayPanPanBasics', '緊急機に対する管制/優先取り扱い', '航空通信/緊急機に対する管制/優先取り扱い'),
  ('PPL-4-3-2_MaydayPanPanBasics', '緊急機に対する管制/緊急機に対する管制', '航空通信/緊急機に対する管制/緊急機に対する管制')
) AS v(learning_content_id, sub_subject, topic_category) ON v.learning_content_id = lc.id
WHERE EXISTS (
  SELECT 1 FROM unified_cpl_questions q
  WHERE q.verification_status = 'verified'
    AND q.main_subject = '航空通信'
    AND q.sub_subject = v.sub_subject
    AND (
      q.applicable_exams @> ARRAY['PPL']::text[]
      OR v.sub_subject LIKE '緊急機に対する管制/%'
    )
  LIMIT 1
)
ON CONFLICT (learning_content_id, topic_category) DO UPDATE SET
  content_title = EXCLUDED.content_title,
  content_category = EXCLUDED.content_category,
  test_question_ids = EXCLUDED.test_question_ids,
  unified_cpl_question_ids = EXCLUDED.unified_cpl_question_ids,
  topic_category = EXCLUDED.topic_category,
  subject_area = EXCLUDED.subject_area,
  mapping_source = EXCLUDED.mapping_source,
  verification_status = EXCLUDED.verification_status,
  updated_at = NOW();
