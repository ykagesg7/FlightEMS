...
8. 火曜・GA4+Sentry 週次レビュー（ISO 週） → 正本 [`docs/ops/Weekly_Telemetry_Review.md`](docs/ops/Weekly_Telemetry_Review.md)。数字は GitHub Actions `weekly-telemetry-ga4` の artifact。Facts 通知は `weekly-telemetry-notify`（`@` メンションなし）。承認コマンドはフェーズ2c（未着手）。認証は [`docs/Cursor_MCP_Setup.md`](docs/Cursor_MCP_Setup.md)
