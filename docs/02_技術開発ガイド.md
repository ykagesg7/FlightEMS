...
- **本番 URL（Production）**: **`https://flight-lms.vercel.app/`** — GA4 データストリームのサイト URL・計測確認の基準とする（運用・トラブルシュートは [04_運用保守ガイド.md](04_運用保守ガイド.md)）
...
アプリ内ギャラリー・ショップ用のページ・フックは削除。`/gallery`・`/shop`・`/experience` のルートは **削除済み**（2026-04-12）。スキーマは [05_設計仕様書.md](05_設計仕様書.md)「DB スリム化」と `scripts/database/20260411_drop_gallery_shop_purchase.sql` を参照。
