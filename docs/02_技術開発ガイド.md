...
**最新更新**: 2026年3月21日（RainViewer / Open-Meteo 外部APIメモを追記）
**バージョン**: Technical Development Guide v3.4
...
- プロジェクト名: **`flight-lms`**（フレームワーク Vite、本番例: `flight-lms.vercel.app`）
...
AI から **実ブラウザ**（コンソール・ネットワーク・スナップショット等）を扱うには、`.cursor/mcp.json` に **`chrome-devtools`** サーバを定義する（[`.cursor/mcp.json.example`](../.cursor/mcp.json.example) 参照）。要件は Node **v20.19+** と **Chrome**。手順・オプトアウト・Slim モードは **[13_Cursor_MCP_Setup.md](13_Cursor_MCP_Setup.md)** の「Chrome DevTools MCP」節を参照。設定変更後は **Cursor の再起動** が必要。
...
アプリ内ギャラリー・ショップ用のページ・フックは削除。`/gallery`・`/shop` はホームへリダイレクト。スキーマは [05_設計仕様書.md](05_設計仕様書.md)「DB スリム化」と `scripts/database/20260411_drop_gallery_shop_purchase.sql` を参照。
