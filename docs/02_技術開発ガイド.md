# FlightAcademyTsx 技術開発ガイド

## 📋 概要

このドキュメントは、FlightAcademyTsxプロジェクトの技術詳細、開発環境、コントリビューション方法、高度な機能、パフォーマンス最適化について包括的に説明します。

**最新更新**: 2025年12月21日 - ThemeContext削除完了、HUD固定スタイル統一、全27ファイル修正完了
**バージョン**: Technical Development Guide v2.4

---

## 🏗️ 技術スタック詳細

### **アーキテクチャ概要**

#### **フロントエンド**
- **React 18**: Concurrent Mode、Suspense、useTransition
- **TypeScript**: 型安全性、開発効率向上
- **Vite**: 高速ビルドツール、動的チャンク分割

#### **バックエンド**
- **Supabase**: データベース、認証、リアルタイム機能
- **PostgreSQL**: リレーショナルデータベース

#### **地図・ナビゲーション**
- **Leaflet**: インタラクティブ地図ライブラリ
- **React-Leaflet**: React用Leafletラッパー
- **GeoJSON**: 地理データ形式

#### **外部API**
- **WeatherAPI.com**: 一般気象情報API（要APIキー）
- **NOAA Aviation Weather Center API**: 航空気象データAPI（認証不要・無料）
  - METAR（定時飛行場実況気象通報式）
  - TAF（飛行場予報）
  - フライトカテゴリー情報

#### **アニメーション・UI**
- **Framer Motion**: Marketingページ用のアニメーションライブラリ
- **Lucide React**: アイコンライブラリ

---

## 🛠️ 開発環境セットアップ

### **必要条件**
- Node.js 16.x以上
- npm 7.x以上

### **インストール手順**

1. **依存関係のインストール**
```bash
npm install
```

2. **環境変数の設定**

`.env.local`ファイルを作成し、以下の内容を追加：

```bash
# WeatherAPI.com（一般気象情報用）
VITE_WEATHER_API_KEY=your_weather_api_key
WEATHER_API_KEY=your_weather_api_key

# Supabase
VITE_SUPABASE_URL=your_supabase_url
VITE_SUPABASE_ANON_KEY=your_supabase_anon_key

# その他のAPI（必要に応じて）
VITE_GOOGLE_API_KEY=your_google_api_key
VITE_GEMINI_API_KEY=your_gemini_api_key
```

3. **開発サーバーの起動**

**推奨: ローカル開発環境（APIサーバー + フロントエンド）**

2つのターミナルを開いて以下を実行：

```bash
# ターミナル1: ローカルAPIサーバー起動
npm run dev:weather

# ターミナル2: フロントエンド開発サーバー起動
npm run dev
```

**オプション: Vercel Dev環境（フルスタック統合）**
```bash
# Vercel CLIのインストール（初回のみ）
npm install -g vercel

# フルスタック開発サーバー起動
npm run dev:full
```
※ Vercel Devは日本語のPC名がある環境で問題が発生する場合があります。

### **開発サーバーの詳細**

#### **ローカルAPIサーバー（`dev:weather`）**
- **ポート**: 3001
- **エンドポイント**:
  - `GET /api/health` - ヘルスチェック
  - `GET /api/weather?lat={lat}&lon={lon}` - 一般気象情報
  - `GET /api/aviation-weather?type={metar|taf}&icao={ICAO}` - METAR/TAF航空気象情報
- **CORS**: 全オリジン許可（開発環境のみ）
- **プロキシ**: Viteが`/api/*`リクエストを自動的に`localhost:3001`に転送

#### **METAR/TAF API仕様**
```typescript
// METARリクエスト
GET /api/aviation-weather?type=metar&icao=RJFF

// TAFリクエスト
GET /api/aviation-weather?type=taf&icao=RJFF

// レスポンス例（METAR）
{
  "icaoId": "RJFF",
  "obsTime": 1761969600,
  "rawOb": "METAR RJFF 010400Z 30015KT 9999 FEW030 SCT040 20/11 Q1011",
  "fltCat": "VFR",
  "temp": 20,
  "dewp": 11,
  "wdir": 300,
  "wspd": 15,
  "clouds": [...]
}
```

### **METAR/TAF統合の特徴**

#### **サーバーサイドプロキシ**
- **CORS問題の解決**: ブラウザから直接NOAA APIにアクセスするとCORSエラーが発生するため、サーバーサイドプロキシを実装
- **ローカル開発**: `scripts/dev-weather-server.ts`が開発環境で動作
- **本番環境**: `api/aviation-weather.ts`がVercel Serverless Functionとして動作

#### **対応空港**
- **日本国内主要空港**: ICAOコードが`RJ`で始まる空港
  - 福岡空港（RJFF）
  - 成田空港（RJAA）
  - 羽田空港（RJTT）
  - 関西空港（RJBB）
  - 新千歳空港（RJCC）
  - など

#### **キャッシュ戦略**
- **有効期間**: 30分（1800秒）
- **ストレージ**: WeatherCacheContext（React Context API）
- **並列取得**: 一般気象情報とMETAR/TAFを同時取得

#### **エラーハンドリング**
- **404エラー**: データが存在しない空港の場合
- **504エラー**: API呼び出しタイムアウト（10秒）
- **フォールバック**: エラー時は適切なメッセージを表示

---

## 🛠️ フロントエンド技術スタック

### **コアフレームワーク**
```json
{
  "react": "^18.2.0",
  "react-dom": "^18.2.0",
  "typescript": "^5.0.0"
}
```

#### **React 18 の主要機能**
- **Concurrent Mode**: 非同期レンダリング
- **Suspense**: ローディング状態管理
- **useTransition**: 優先度付き更新
- **Automatic Batching**: 自動バッチング

### **ビルドツール**
```json
{
  "vite": "^4.4.0",
  "@vitejs/plugin-react": "^4.0.0"
}
```

#### **Vite の最適化機能**
- **動的チャンク分割**: コード分割の最適化
- **HMR (Hot Module Replacement)**: 高速開発
- **ESBuild**: 高速バンドリング

### **UI・スタイリング**
```json
{
  "tailwindcss": "^3.3.0",
  "react-select": "^5.7.0",
  "@headlessui/react": "^1.7.0",
  "framer-motion": "^12.23.24",
  "lucide-react": "^0.344.0"
}
```

#### **Tailwind CSS**
- **ユーティリティファースト**: 効率的なスタイリング
- **レスポンシブデザイン**: モバイルファースト
- **カスタムテーマ**: ダークモード対応
- **Whisky Papaブランドカラー**: Yellow (#FFD700), Black (#121212) の追加

### **デザインシステムとテーマ戦略（2025年12月実装）**

#### **デュアルテーマアーキテクチャ**
アプリケーションを「魅せる（Marketing）」と「使う（App）」の2つのコンテキストに分離し、CSS変数を用いたセマンティックカラーで制御。

1.  **Marketing Theme (Whisky Papa Brand)**
    - **用途**: LP, About, Shop, Experience
    - **カラー**: Brand Yellow (`#FFD700`) & Black (`#121212`)
    - **フォント**: Impact, Arial Black (Display)

2.  **App Theme (Flight Academy HUD)**
    - **用途**: Dashboard, Learning, Tools
    - **Day Mode**: Navy Blue (`#0b1d3a`) & HUD Green (`#39FF14`)
    - **Dark Mode**: Black (`#000000`) & HUD Red (`#ff3b3b`) - コックピット夜間照明模倣

#### **共通UIコンポーネント (`src/components/ui/`)**
Tailwindの`variant`プロップスを活用し、文脈に応じてスタイルを切り替える基盤コンポーネント。

- **`Button`**: `variant="brand" | "hud" | "primary" | "secondary"`
- **`Card`**: `variant="brand" | "hud" | "default"`
- **`Typography`**: `variant="display" | "h1"-"h6" | "body" | "mono"`

---

#### **Framer Motion**
- **アニメーション**: Marketingページ用のスムーズなアニメーション
- **パフォーマンス**: GPU加速による高速レンダリング
- **アクセシビリティ**: `prefers-reduced-motion` 対応

#### **検索・フィルタリング機能**
- **ArticleSearch**: 記事検索とタグフィルタリングの統合コンポーネント
- **正規化検索**: 全角半角・かな統一による日本語検索最適化
- **タグシステム**: sub_categoryベースの動的タグ抽出

#### **LearningページUI統合（更新）**
- **Articlesページ準拠**: 統一された検索・タグUIの実装
- **ダッシュボード維持**: 既存の「今日の復習」「続きから」「おすすめ」機能を保持
- **CPL学科コンテンツ**: 航空法規・航空工学・航空気象シリーズの統合表示（3.3.*を「航空気象」として自動タグ推定）

#### **新規共通UI（2025-08）**
- `components/common/FilterTabs.tsx`: A11y対応のカテゴリタブ（左右キー、`role="tablist"`）
- `components/common/SearchAndTags.tsx`: 検索＋タグ、URL同期と相性の良い `value`/`tags` props
- `components/common/ProgressRing.tsx`: SVGリング。`animejs` を遅延importしてストロークをアニメ
- `hooks/useAnimeInView.ts`: IntersectionObserver＋遅延importのスクロールイン演出
- `components/lessons/LessonCard.tsx`: 進捗リング＋完了バッジ＋関連TEST CTA。記事詳細は `/learning/:contentId` に遷移

### **状態管理**
```json
{
  "zustand": "^4.4.0",
  "@tanstack/react-query": "^4.29.0"
}
```

#### **Zustand**
- **軽量**: バンドルサイズ最小
- **型安全**: TypeScript対応
- **シンプル**: 学習コスト低

#### **React Query**
- **サーバー状態管理**: キャッシュ、同期
- **バックグラウンド更新**: 自動更新
- **エラーハンドリング**: 包括的エラー処理

---

## 📝 ファイルエンコーディング規約（重要）

### **エンコーディング統一**
- **必須**: すべてのコードファイル、MDXファイル、ドキュメントは「UTF-8（BOMなし）」で保存
- **禁止**: UTF-16、Shift-JIS、CP932エンコーディングの使用は禁止

### **MDXファイル規約**
- **フロントマター**: メタデータの統一形式
- **画像パス**: `/images/ContentImages/` ディレクトリ配下
- **コンポーネント**: カスタムMDXコンポーネントの活用

---

## 🗄️ データベース設計

### **Whisky Papa ゲーミフィケーション**

#### **profiles テーブル拡張**
```sql
ALTER TABLE profiles
ADD COLUMN rank user_rank_type DEFAULT 'spectator',
ADD COLUMN xp_points INTEGER DEFAULT 0;

-- パスワード更新日時カラム追加（2025年12月21日）
ALTER TABLE profiles
ADD COLUMN IF NOT EXISTS password_updated_at timestamp with time zone DEFAULT NULL;

CREATE INDEX IF NOT EXISTS idx_profiles_password_updated_at ON profiles(password_updated_at);
```

#### **missions テーブル**
```sql
CREATE TABLE missions (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  title TEXT NOT NULL,
  description TEXT,
  required_action TEXT NOT NULL,
  xp_reward INTEGER DEFAULT 100,
  min_rank_required user_rank_type DEFAULT 'spectator',
  mission_type TEXT DEFAULT 'one_time',
  is_active BOOLEAN DEFAULT true,
  created_at TIMESTAMPTZ DEFAULT now(),
  updated_at TIMESTAMPTZ DEFAULT now()
);

-- RLS（Row Level Security）の設定
ALTER TABLE missions ENABLE ROW LEVEL SECURITY;

-- 全ユーザーがアクティブなミッションを閲覧可能
CREATE POLICY "missions_select_active" ON missions
  FOR SELECT
  USING (is_active = true);

-- Adminは全ミッションを閲覧可能
CREATE POLICY "missions_admin_select_all" ON missions
  FOR SELECT
  USING (
    (select auth.uid()) IN (
      SELECT profiles.id FROM profiles WHERE profiles.roll = 'Admin'
    )
  );

-- Adminはミッションを管理可能（INSERT/UPDATE/DELETE）
CREATE POLICY "missions_admin_insert" ON missions FOR INSERT ...
CREATE POLICY "missions_admin_update" ON missions FOR UPDATE ...
CREATE POLICY "missions_admin_delete" ON missions FOR DELETE ...
```

#### **user_missions テーブル**
```sql
CREATE TABLE user_missions (
  user_id UUID REFERENCES profiles(id) ON DELETE CASCADE,
  mission_id UUID REFERENCES missions(id) ON DELETE CASCADE,
  completed_at TIMESTAMPTZ DEFAULT now(),
  xp_earned INTEGER DEFAULT 0,
  PRIMARY KEY (user_id, mission_id)
);

-- RLS（Row Level Security）の設定
ALTER TABLE user_missions ENABLE ROW LEVEL SECURITY;

-- ユーザーは自分のミッション達成記録を閲覧可能
CREATE POLICY "user_missions_select_own" ON user_missions
  FOR SELECT
  USING ((select auth.uid()) = user_id);

-- ユーザーは自分のミッション達成記録を挿入可能
CREATE POLICY "user_missions_insert_own" ON user_missions
  FOR INSERT
  WITH CHECK ((select auth.uid()) = user_id);

-- Adminは全ユーザーのミッション達成記録を管理可能
CREATE POLICY "user_missions_admin_select_all" ON user_missions
  FOR SELECT
  USING (
    (select auth.uid()) IN (
      SELECT profiles.id FROM profiles WHERE profiles.roll = 'Admin'
    )
  );
```

### **Whisky Papa エンゲージメント**

#### **products テーブル**
```sql
CREATE TABLE products (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  name TEXT NOT NULL,
  description TEXT,
  price INTEGER NOT NULL,
  image_url TEXT,
  required_rank user_rank_type DEFAULT 'spectator',
  external_link TEXT,
  is_available BOOLEAN DEFAULT true,
  stock_count INTEGER,
  created_at TIMESTAMPTZ DEFAULT now(),
  updated_at TIMESTAMPTZ DEFAULT now()
);
```

#### **fan_photos テーブル**
```sql
CREATE TABLE fan_photos (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id UUID REFERENCES profiles(id) ON DELETE CASCADE,
  image_url TEXT NOT NULL,
  caption TEXT,
  is_approved BOOLEAN DEFAULT false,
  is_featured BOOLEAN DEFAULT false,
  approved_by UUID REFERENCES profiles(id),
  approved_at TIMESTAMPTZ,
  created_at TIMESTAMPTZ DEFAULT now(),
  updated_at TIMESTAMPTZ DEFAULT now()
);
```

### **学習コンテンツ管理**
```sql
-- learning_contents テーブル
CREATE TABLE learning_contents (
  id VARCHAR PRIMARY KEY,
  title VARCHAR NOT NULL,
  category VARCHAR NOT NULL,
  sub_category VARCHAR,  -- タグ機能用
  description TEXT,
  order_index INTEGER,
  parent_id VARCHAR,
  content_type VARCHAR DEFAULT 'article',
  is_published BOOLEAN DEFAULT true,
  created_at TIMESTAMP DEFAULT NOW(),
  updated_at TIMESTAMP DEFAULT NOW()
);
```

#### **CPL学科コンテンツ構造**
```sql
-- 3.1.x 航空法規シリーズ
UPDATE learning_contents SET sub_category = '航空法規'
WHERE id IN ('3.1.1_AviationLegal0', '3.1.2_AviationLegal1', '3.1.3_AviationLegal2', '3.1.4_AviationLegal3');

-- 3.2.x 航空工学シリーズ
UPDATE learning_contents SET sub_category = '航空工学'
WHERE id IN ('3.2.1_PropellerTheory', '3.2.2_WingTheory', '3.2.3_StabilityControl',
             '3.2.4_HydraulicElectrical', '3.2.5_EngineTheory', '3.2.6_InstrumentSystem');
```

### **学習テストマッピング**
```sql
-- learning_test_mapping テーブル
CREATE TABLE learning_test_mapping (
  learning_content_id VARCHAR REFERENCES learning_contents(id),
  content_title VARCHAR NOT NULL,
  content_category VARCHAR NOT NULL,
  topic_category VARCHAR NOT NULL,
  subject_area VARCHAR NOT NULL,
  relationship_type VARCHAR DEFAULT 'direct',
  weight_score DECIMAL DEFAULT 1.0,
  difficulty_level INTEGER DEFAULT 1,
  estimated_study_time INTEGER DEFAULT 30,
  mapping_source VARCHAR DEFAULT 'manual',
  confidence_score DECIMAL DEFAULT 0.8,
  verification_status VARCHAR DEFAULT 'pending',
  created_at TIMESTAMP DEFAULT NOW(),
  updated_at TIMESTAMP DEFAULT NOW(),
  PRIMARY KEY (learning_content_id, topic_category, relationship_type)
);
```

---

## ⚡ パフォーマンス最適化

### **React 18 Concurrent Features**
- **useTransition**: 非緊急更新の遅延処理
- **Suspense**: ローディング状態の管理
- **Automatic Batching**: 自動バッチング

#### **検索・フィルタリング最適化**
- **useMemo**: フィルタリング結果のメモ化
- **正規化関数**: 検索クエリの事前処理
- **動的タグ抽出**: 利用可能タグの効率的な計算

### **バンドルサイズ最適化**
- **動的インポート**: コード分割
- **Tree Shaking**: 未使用コードの除去
- **画像最適化**: WebP形式の活用

---

## 🧩 コンポーネント設計

### **再利用可能コンポーネント**

#### **UI基盤コンポーネント（`src/components/ui/`）**
- **Button**: マルチバリアント対応ボタン（Brand/HUD/Ghost等）
- **Card**: セマンティックカラー対応カードコンポーネント
- **Typography**: 統一されたフォントスケールとカラーシステム

#### **既存コンポーネント**
- **ArticleCard**: 記事カード表示
- **ArticleSection**: 記事セクション管理
- **ArticleSearch**: 検索・タグフィルタリング（Articles/Learning共通）
- **QuizComponent**: クイズ表示・解答
- **MDXLoader**: MDXコンテンツ表示

#### **Whisky Papa Marketingコンポーネント**
- **RankBadge**: ユーザーランク表示バッジ（Spectator/Trainee/Wingman）
- **MissionCard**: ミッションカード（達成状況、XP報酬表示）
- **ComingSoonBadge**: Coming Soon表示バッジ

#### **Shopコンポーネント**
- **ProductCard**: 商品カード（ランクロック機能付き）

#### **Galleryコンポーネント**
- **PhotoGrid**: 写真グリッド表示（Masonryレイアウト、Lightbox機能、いいね機能）
- **UploadModal**: 写真アップロードモーダル（ドラッグ&ドロップ対応）
- **AdminEventPanel**: イベント管理パネル（管理者専用、イベント作成・アクティブ化）
- **AdminReviewPanel**: 承認管理パネル（管理者専用、承認待ち写真の承認・却下・おすすめ設定）

#### **Blogコンポーネント**
- **NarratorTemplate**: Junさん（ナレーター）専用の脚本風ブログテンプレート

### **Learningページ専用コンポーネント**
- **RelatedTestButton**: 関連テストへの遷移
- **ReviewContentLink**: 復習コンテンツへのリンク

### **ダッシュボード専用コンポーネント（2025年1月16日実装）**
- **DailyTasks**: 今日の学習タスクを自動生成・表示
  - 弱点トピック、SRS復習、続きから学習レッスンを優先度順で表示
  - データソース: `user_test_results`, `user_unified_srs_status`, `learning_progress`
- **SubjectRadarChart**: 科目別理解度をレーダーチャートで可視化
  - Chart.js Radarチャート使用
  - 対象科目: 航空法規、航空工学、航空気象、空中航法、航空通信
- **LearningHeatmap**: 過去90日間の学習履歴をヒートマップ形式で表示
  - カスタムSVGベースの実装
  - 学習時間に応じた強度レベル（0-3）で色分け表示
- **EnhancedLearningDashboard**: 学習ダッシュボード

---

## 🔄 状態管理

### **Zustand Store**
```typescript
// authStore.ts
interface AuthState {
  user: User | null;
  session: Session | null;
  loading: boolean;
}
```

#### **Learningページ状態管理**
```typescript
// LearningPage.tsx
interface LearningState {
  searchQuery: string;
  selectedTags: string[];
  filteredContents: LearningContent[];
  availableTags: string[];
}
```

### **React Query**
- **useQuery**: データ取得とキャッシュ
- **useMutation**: データ更新
- **useInfiniteQuery**: 無限スクロール

### **Whisky Papa専用Hooks**
- **useGamification**: ランク・XP・ミッション管理
  - プロフィール取得、ミッション達成処理、ランクアップ判定
  - **修正（2025年12月21日）**: `user_missions`クエリで`.single()`から`.limit(1)`に変更し、エラーハンドリングを改善
- **useShop**: 商品一覧取得とランクチェック
- **useGallery**: ギャラリー写真の取得とアップロード
  - イベント管理（アクティブイベント・アーカイブイベント）
  - いいね機能（承認済み写真のみ、アクティブイベントのみ）
  - 写真の承認・却下・おすすめ設定（管理者）
  - Top3表示、ソート機能（新着順・いいね順）

#### **学習コンテンツ取得**
```typescript
// useLearningProgress.ts
const { learningContents, isLoading, error } = useLearningProgress();

// CPL学科コンテンツフィルタリング（航空法規/航空工学/航空気象）
const cplLearningContents = useMemo(() => {
  return learningContents.filter(content =>
    content.is_published && content.category === 'CPL学科'
  );
}, [learningContents]);
```

---

## 🛡️ エラーハンドリング

### **Error Boundary**
- **EnhancedErrorBoundary**: 包括的エラー処理
- **フォールバックUI**: エラー時の代替表示
- **エラーログ**: 詳細なエラー情報の記録

#### **検索・フィルタリングエラー処理**
- **正規化エラー**: 検索クエリの不正文字処理
- **タグ抽出エラー**: sub_category未設定時のID接頭辞推定
- **フィルタ結果エラー**: 空結果時の適切なメッセージ表示

---

## 🔒 セキュリティ

### **認証・認可**
- **Supabase Auth**: ユーザー認証
- **RLS (Row Level Security)**: 行レベルセキュリティ
- **環境変数**: 機密情報の管理

#### **コンテンツアクセス制御**
- **is_published**: 公開状態の管理
- **category**: カテゴリ別アクセス制御
- **sub_category**: タグベースのフィルタリング

---

## 🧪 テスト戦略

### **単体テスト**
- **Vitest**: 高速テスト実行
- **React Testing Library**: コンポーネントテスト
- **TypeScript**: 型安全性テスト

#### **検索・フィルタリングテスト**
- **正規化関数テスト**: 日本語検索の正確性
- **タグ抽出テスト**: sub_categoryとID接頭辞の処理
- **フィルタリングテスト**: 複数条件の組み合わせ

---

## 🚀 デプロイメント

### **Vercel**
- **自動デプロイ**: GitHub連携
- **環境変数**: 本番環境設定
- **パフォーマンス監視**: Core Web Vitals

#### **データベース更新**
- **SupabaseMCP**: マイグレーション実行
- **SQL実行**: コンテンツ登録・更新
- **データ整合性**: スキーマ・データの同期

---

## 🔄 開発ワークフロー

### **Git Flow**
- **main**: 本番環境
- **develop**: 開発環境
- **feature**: 機能開発

#### **コンテンツ更新ワークフロー**
1. **MDX作成**: 新しい学習コンテンツの作成
2. **SQL生成**: データベース登録用SQLの作成
3. **SupabaseMCP実行**: データベースへの反映
4. **フロントエンド確認**: 表示・検索・タグ機能の動作確認

---

## 📊 パフォーマンス監視

### **Core Web Vitals**
- **LCP (Largest Contentful Paint)**: 最大コンテンツ描画時間
- **FID (First Input Delay)**: 初回入力遅延
- **CLS (Cumulative Layout Shift)**: 累積レイアウトシフト

#### **検索・フィルタリングパフォーマンス**
- **検索応答時間**: 100ms以内の検索結果表示
- **タグ抽出時間**: 利用可能タグの即座な計算
- **フィルタリング時間**: 複数条件での高速フィルタリング

---

## 📚 最近の実装（2025年1月）

### **Whisky Papa Transformation Phase 1-4実装**

#### **Phase 1: Brand Foundation**
- MarketingLayoutとAppLayoutの実装
- Whisky Papaブランドカラー（Yellow, Black）の追加
- Home/Aboutページの実装
- Framer Motionによるアニメーション実装

#### **Phase 2: Gamification**
- データベーススキーマ拡張（ランク・XP・ミッション）
- useGamification Hookの実装
- Mission Dashboardページの実装
- 既存機能との統合（クイズ・記事読了時の自動ミッション達成）

#### **Phase 3: Engagement**
- Shop機能（ランク連動型商品表示）
- Gallery機能（写真アップロード・承認制）
- Blog機能（ナレーターブログテンプレート）

#### **Phase 4: Real Experience**
- Coming Soonページの実装
- Wingmanランク限定の表示ロジック

## 📚 最近の実装（2025年8月17日〜）

### **Lessonページ検索・タグ機能統合**
- **ArticleSearch統合**: Articlesページと同等の検索・タグUIを実装
- **CPL学科コンテンツ対応**: 航空法規・航空工学シリーズの統合表示
- **ダッシュボード機能維持**: 既存の学習進捗機能を保持

### **CPL学科コンテンツ登録**
- **航空法規シリーズ**: 3.1.1〜3.1.4（4記事）
- **航空工学シリーズ**: 3.2.1〜3.2.6（6記事）
- **sub_category設定**: タグ機能のための適切な分類
- **航空気象シリーズ**: 3.3.1（W-1）, 3.3.2（W-2）を追加（カテゴリ `CPL学科`、サブカテゴリ `航空気象`）

### **SupabaseMCP活用**
- **データベース更新**: 10記事の一括登録・更新
- **マイグレーション実行**: 安全なスキーマ変更
- **データ整合性確認**: 登録後の動作確認

### **技術的改善点**
- **検索正規化**: 日本語検索の最適化
- **タグ動的抽出**: sub_categoryベースの自動タグ生成
- **フィルタリング最適化**: useMemoによるパフォーマンス向上

## 📝 変更履歴（2025年12月21日）

### **UI/UX統一**
- **HUDテーマ削除**: `ThemeContext`、`HudPrimitives`、`ThemeToggler`、`AppLayout`を削除
- **Whisky Papaテーマ統一**: 全ページを`MarketingLayout`で統一
- **ハンバーガーメニュー実装**: モバイル対応のナビゲーションメニュー
- **Missionページ統合**: FlightPlanner、Articles、CPL Quiz、Academyへのアクセスを「Tools」タブに統合

### **データベース修正**
- **complete_mission関数**: `ON CONFLICT DO UPDATE`句で`EXCLUDED.xp_earned`を使用（曖昧性回避）
- **テスト結果保存**: `quiz_sessions`への挿入後に`session_id`を取得し、`user_test_results`に紐付け

### **フック修正**
- **useGamification**: `user_missions`クエリで`.single()`から`.limit(1)`に変更、エラーハンドリング改善

---

## 🔮 今後の技術ロードマップ

### **短期目標（1-2ヶ月）**
- **パフォーマンス最適化**: 検索・フィルタリングの高速化
- **ユーザビリティ向上**: 検索結果のハイライト機能
- **モバイル対応**: タグUIのレスポンシブ改善

### **中期目標（3-6ヶ月）**
- **高度な検索**: 全文検索エンジンの導入
- **レコメンデーション**: 学習履歴ベースの記事推薦
- **オフライン対応**: Service Workerによるキャッシュ

### **長期目標（6ヶ月以上）**
- **AI機能**: 自然言語検索の実装
- **パーソナライゼーション**: ユーザー別学習パス
- **分析機能**: 学習効果の可視化

---

## 📝 変更履歴

### **Gallery機能の段階的実装（2025年12月26日）**

#### **Phase 1: 管理タブ拡張**
- **AdminReviewPanelコンポーネント**: 管理者専用の承認管理パネルを実装
  - 承認待ち写真の一覧表示、承認・特集設定・却下機能
- **Galleryページタブ機能**: 公開ギャラリー・管理タブの切り替え

#### **Phase 2: アップロード機能復旧**
- **Supabase Storage設定**: `fan-photos`バケットを作成（5MB制限、画像MIMEタイプ制限）
- **Storage RLSポリシー**: 適切な閲覧・アップロード・削除権限を設定
- **UploadModal修正**: スクロール不能バグを修正、エラーメッセージ改善

#### **Phase 3: いいね機能・Masonryレイアウト・自分の投稿タブ**
- **いいね機能実装**: `useGallery`フックにいいね数取得・ユーザーいいね状態取得・トグル機能を追加
- **PhotoGrid拡張**: いいねUIを追加（❤️/🤍アイコンと件数表示）
- **人気Top3セクション**: いいね数上位3件を表示
- **ソート機能**: 新着順・いいね順の切り替え
- **自分の投稿タブ**: 承認待ち/承認済みの分類表示
- **Masonryレイアウト**: CSS columnsベースの実装（依存追加なし）

#### **Phase 4: イベント制Gallery実装**
- **gallery_eventsテーブル**: イベント管理テーブルを作成
- **fan_photosテーブル拡張**: `event_id`フィールドを追加
- **アクティブイベント制約**: 常に1つのみアクティブ（自動アーカイブ機能）
- **投稿上限機能**: 1ユーザー1イベント5枚制限（Adminは無制限）
- **いいねルール強化**: RLSポリシーでDB側制約を追加
- **AdminEventPanel拡張**: イベント作成・アクティブ化UIを追加
- **Galleryページ構成変更**: イベント単位の表示に刷新
- **UploadModal拡張**: イベント連動とガイド表示を追加

### **v1.1 (2025年9月17日) - Articlesページテーマ統一・登録ボタン有効化**

#### **🎨 テーマシステムの統一**
- **学習進捗ダッシュボードへのDay/Dark/Autoテーマ適用**
  - `ProgressSummaryHeader`、`ProgressSidebar`、`EnhancedArticleCard`、`ReadingTimeEstimate`コンポーネントで`theme`を`effectiveTheme`に統一
  - Dayテーマ：NavyBlue背景、白文字、HUDグリーン枠線
  - Darkテーマ：コックピット赤色照明デザイン（`hud-surface`、`shadow-red-900/20`）

#### **🎯 Articlesページデザイン統一**
- **HUDスタイルの適用**
  - `ArticleDashboard`：NavyBlue背景（`#0b1d3a`）、HUD枠線（`hud-line`）
  - 記事カード：`hud-surface`背景、`hover:bg-white/10`ホバー効果
  - 検索バー、タブ、統計カード：HUDスタイルに統一

#### **📊 学習進捗カードのデザイン統一**
- **統計カードのHUD化**
  - 読了記事数、学習時間、連続学習日数、今週の進捗カード
  - 今日の目標セクション、達成済みバッジ
  - カテゴリー別進捗、シリーズ別進捗、最近の活動カード

#### **🔍 分析カードのデザイン統一**
- **「更に詳しい学習分析を体験」カード**
  - Dayテーマ：`hud-surface hover:bg-white/10 shadow-green-900/10`
  - テキスト：`from-[#39FF14] to-green-600`グラデーション
  - ボタン：`hud-surface hud-text hover:bg-white/10`

- **「もっと詳しい分析を見る」カード**
  - Dayテーマ：`hud-surface hover:bg-white/10`
  - Darkテーマ：`hud-surface hover:bg-white/10 shadow-red-900/20`
  - ボタン：`hud-surface hud-text hover:bg-white/10`

#### **🔐 登録ボタンの有効化**
- **ログイン画面への遷移機能**
  - `ArticleDashboard`：`useNavigate`を追加、`showRegistrationModal`で`/auth`へ遷移
  - 全登録ボタン（「✨ 無料で登録」「無料で始める」「登録して見る」「もっと詳しい分析を見る」）が有効化
  - 既存の`/auth`ルートを使用した遷移実装

#### **🛠️ 技術的改善**
- **テーマ一貫性の確保**
  - 全コンポーネントで`effectiveTheme`の使用を統一
  - CSS変数（`--hud-primary`、`--hud-secondary`、`--hud-surface`）の活用
  - 条件付きレンダリングの最適化

- **エラーハンドリングの強化**
  - `ReferenceError: theme is not defined`の修正
  - 全コンポーネントでの`theme ===`を`effectiveTheme ===`に置換
  - Linterエラーの解消

#### **📈 パフォーマンス最適化**
- **コンポーネント最適化**
  - `useCallback`による関数のメモ化
  - 条件付きレンダリングの効率化
  - 不要な再レンダリングの防止

### **v1.1 (2025年9月23日) - Articlesページテーマ統一とカードデザイン統一**

#### **🎨 Articlesページテーマ統一**
- **Day/Dark/Autoテーマの完全適用**
  - ローディング画面：HUDスタイルの背景とボーダーを適用
  - 記事一覧グリッド：Dayテーマで`hud-grid`クラスを適用
  - 「記事が見つかりませんでした」メッセージ：HUDスタイルの背景とボーダーを適用

#### **🔧 記事フィルタリング・並び替えカードのHUDスタイル適用**
- **カードの枠線とシャドウ**
  - Darkテーマ：`border-red-500/60 shadow-red-900/20` - コックピット赤色照明デザイン
  - Dayテーマ：`border-green-500/50 shadow-green-900/10` - HUDグリーンスタイル
- **フォーム要素のスタイル統一**
  - selectボックス：`border-2` + テーマ対応の枠線色
  - ソート順序ボタン：`border-2` + テーマ対応の枠線色
  - カテゴリータブ・タグボタン：`border-2`で統一

#### **📐 枠線の太さ統一**
- **統一された枠線**
  - 太さ：すべて`border-2`（2px）で統一
  - 色：テーマに応じた適切な色を維持
  - 視認性：より鮮明で一貫したHUDスタイル

#### **🎯 Articlesページ全体のカードデザイン統一**
- **統一されたデザイン要素**
  - 角丸：すべて`rounded-xl`に統一
  - 枠線：すべて`border-2`（2px）に統一
  - シャドウ：すべて`shadow-lg`に統一
  - パディング：適切なパディング（`p-5`または`p-3 sm:p-4`）に統一
  - テーマ対応：すべてのカードでテーマに応じた色を適用

#### **📊 修正されたコンポーネント**
- **ArticleSearch**：`rounded-lg` → `rounded-xl`、`p-4` → `p-5`
- **ArticleSortControls**：`rounded-lg` → `rounded-xl`、`p-4` → `p-5`
- **ProgressSidebar**：すべてのカードに`border-2`とテーマ対応の枠線色を追加
- **EnhancedArticleCard**：`border` → `border-2`、テーマ対応の枠線色を追加
- **ProgressSummaryHeader**：`rounded-2xl` → `rounded-xl`、`shadow-2xl` → `shadow-lg`、`border-2`を追加、統計カードにも枠線とシャドウを追加

#### **🎨 統一されたテーマスタイル**
- **Darkテーマ**：`border-red-500/60 shadow-red-900/20` - コックピット赤色照明デザイン
- **Dayテーマ**：`border-green-500/50 shadow-green-900/10` - HUDグリーンスタイル

### **v1.3 (2025年9月23日) - 記事カード統計データ表示統一とログイン制限機能実装**

#### **🔧 統計データ表示の統一**
- **統計データ読み込み修正**: `content_id`をキーとして使用するように修正
- **記事コンテンツロード後の統計データ読み込み**: `useEffect`を追加して適切なタイミングで統計データを読み込み
- **デフォルト値の提供**: 統計データが存在しない記事でも統一された表示を確保

#### **🔐 ログイン状態に応じた表示制御**
- **いいね・コメント数**: ログインユーザーのみ表示
- **閲覧数**: 常に表示（ログイン不要）
- **ログイン状態確認**: `useAuth`フックを使用してログイン状態を判定

#### **📊 閲覧数の自動カウント機能**
- **記事ページ訪問時の閲覧数記録**: 記事タイトルクリック時に自動的に閲覧数を記録
- **`onArticleClick`プロパティ**: `EnhancedArticleCard`に追加して閲覧数記録を実装

#### **🚫 いいね・コメント機能の制限**
- **いいね機能**: ログインユーザーのみ利用可能、匿名ユーザーにはログイン促進メッセージを表示
- **コメント機能**: ログインユーザーのみ利用可能、匿名ユーザーにはログイン促進メッセージを表示
- **匿名ユーザー向け機能削除**: セッション管理によるいいね機能を削除

#### **📅 統一された表示の確保**
- **公開日の統一表示**: `articleMeta.publishedAt`または`article.created_at`を使用
- **統計データの統一表示**: 統計データが0の場合でも「0」として表示
- **メタデータの一貫性**: すべての記事カードで同じレベルのメタデータを表示

#### **🛠️ 修正されたコンポーネント**
- **ArticleDashboard.tsx**: 統計データ読み込みロジックの修正、記事クリック時の閲覧数記録機能追加
- **EnhancedArticleCard.tsx**: ログイン状態に応じた表示制御、公開日の統一表示
- **useArticleStats.ts**: いいね機能をログインユーザーのみに制限、匿名ユーザー向け機能削除

### **v1.2 (2025年9月23日) - Articlesページテーマ統一とカードデザイン統一**
- Articlesページのテーマ統一（Day/Dark/Auto）
- カードデザインの統一（HUDスタイル）
- 枠線の太さ統一（border-2）
- 登録ボタンの有効化

### **v1.0 (2025年8月17日) - Lessonページ検索・タグ機能統合**
- Lessonページの検索・タグ機能統合
- CPL学科コンテンツ登録
- SupabaseMCP活用

---

## 📖 参考資料

### **技術ドキュメント**
- [React 18 Documentation](https://react.dev/)
- [Supabase Documentation](https://supabase.com/docs)
- [Vite Documentation](https://vitejs.dev/)

### **設計パターン**
- **Atomic Design**: コンポーネント設計
- **Container/Presentational**: 状態管理パターン
- **Custom Hooks**: ロジックの再利用

### **ベストプラクティス**
- **TypeScript**: 型安全性の確保
- **Performance**: 最適化手法
- **Security**: セキュリティ対策
