...
- **リダイレクト**: `/blog` と `/blog/:slug` → `/articles`（`:slug` は引き継がない）、`/learning` → `/articles`、`/account` → `/profile`（`/shop`・`/gallery`・`/experience` は **ルート削除済み**・未定義パスは 404）
- **コード分割**: React.lazyによる動的インポート
- **Vercel SPA フォールバック**: `vercel.json` の `index.html` フォールバックは **`/assets/` 配下を除外**（存在しないチャンク URL に HTML が返り `text/html` MIME エラーになるのを防ぐ）。`index.html` は `Cache-Control: max-age=0, must-revalidate` でシェルが古く残りにくいようにしている
...
### **撤去済みレガシー（戦略上はロードマップ外）**

旧 Hangar Store・体験搭乗・アプリ内ギャラリーは **2026-04-12 ルート・UI から削除**（[00](00_Flight_Academy_Strategy.md) §6）。`products` / `purchase_history` / `fan_photos` 等は DB から削除済み。
