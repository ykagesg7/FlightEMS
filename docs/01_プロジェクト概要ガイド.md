...
**最終更新**: 2025年12月24日（リポジトリ整理作業完了）
**バージョン**: Project Overview Guide v2.1
...
- **アニメーション**: スタッガーアニメーションとrevealエフェクト
- **アクセシビリティ**: 全ユーザー（匿名含む）が読み取り可能なRLSポリシー
...
- **ランクシステム**: Spectator → Trainee → Wingman の3段階ランク
- **XPシステム**: ミッション達成でXP獲得、ランクアップ判定
- **ミッションシステム**:
  - クイズ完了、記事読了、プラン作成などのアクションで自動達成
  - ワンタイム、デイリー、ウィークリーミッション対応
- **Mission Dashboard**: ランク・XP・ミッション進捗の可視化

#### **Phase 3: Engagement (Shop & Gallery)**
- **Hangar Store**: ランク連動型ショップ
  - ランクに応じた商品の表示/ロック機能
  - 外部決済サービス（BASE等）へのリンク導線
- **Interactive Gallery**: ファン投稿写真ギャラリー
  - 写真アップロード（ドラッグ&ドロップ対応）
  - 管理者承認制
  - Lightbox機能付きグリッド表示
  - **イベント管理機能**: アクティブイベントとアーカイブイベントの管理
  - **いいね機能**: 承認済み写真へのいいね（アクティブイベントのみ）
  - **Top3表示**: いいね数上位3件の表示
  - **ソート機能**: 新着順・いいね順の切り替え
  - **タブ機能**: 公開ギャラリー・自分の投稿・管理パネル
- **Sky Notes Blog**: ナレーターブログ
  - Junさん（ナレーター）専用の脚本風テンプレート
  - MDXコンテンツ対応

#### **Phase 4: Real Experience (Coming Soon)**
- **体験搭乗予約**: Wingmanランク限定のComing Soonページ
  - ランクチェック機能
  - 機能プレビュー（予約フォーム、招集命令、体験の流れ）
...
- **Framer Motion**: アニメーションライブラリ（Marketingページ用）
- **Zustand**: 軽量状態管理
...
| パス | コンポーネント | 説明 |
|------|---------------|------|
| `/` | Home | Whisky Papaランディングページ |
| `/about` | About | チーム紹介・機体スペック |
| `/mission` | MissionDashboard | ミッションダッシュボード（ランク・XP・ミッション） |
| `/shop` | Shop | ランク連動型ショップ |
| `/gallery` | Gallery | ファン投稿ギャラリー |
| `/blog` | Blog | ブログ一覧 |
| `/blog/:slug` | BlogDetail | ブログ記事詳細 |
| `/experience` | Experience | 体験搭乗予約（Coming Soon） |

**App Routes (Flight Academy Tools)**
| パス | コンポーネント | 説明 |
|------|---------------|------|
| `/dashboard` | HomePage | ダッシュボード（旧ホーム） |
| `/planning` | PlanningMapPage | 飛行計画マップ |
| `/learning` | LearningPage | 学習コンテンツ一覧・閲覧 |
| `/learning/:contentId` | LessonDetailPage | 学習記事の詳細 |
| `/articles` | ArticlesPage | 記事一覧・検索 |
| `/articles/:contentId` | ArticleDetailPage | 記事詳細 |
| `/account` | AccountCenterPage | アカウント管理 |
...
### **Gallery機能の段階的実装完了（2025年12月26日）**

#### **Phase 1: 管理タブ拡張**
- **AdminReviewPanel**: 管理者専用の承認管理パネルを実装
...
- **いいね機能**: 承認済み写真へのいいね機能を実装
- **人気Top3表示**: いいね数上位3件を表示
- **ソート機能**: 新着順・いいね順の切り替え
- **自分の投稿タブ**: 承認待ち/承認済みの分類表示
- **Masonryレイアウト**: CSS columnsベースの実装（依存追加なし）

#### **Phase 4: イベント制Gallery実装**
- **イベント管理**: `gallery_events`テーブルでイベント単位の管理を実現
  - アクティブイベントは常に1つのみ（自動アーカイブ機能）
  - 投票締切日時の設定と制御
- **投稿上限**: 1ユーザー1イベント5枚制限（Adminは無制限）
- **いいねルール強化**: 自分の投稿へのいいね禁止、締切後の投票禁止をDB側で強制
- **AdminEventPanel**: イベント作成・アクティブ化UIを追加
- **Galleryページ構成**: イベント単位の表示に刷新（上部=アクティブ、下部=過去イベント）

### **Whisky Papa Transformation Phase 1-4実装完了（2025年1月）**
- **Phase 1: Brand Foundation**: MarketingLayout、Home/Aboutページ、ブランドカラー実装
- **Phase 2: Gamification**: ランク・XP・ミッションシステムの完全実装
- **Phase 3: Engagement**: Shop、Gallery、Blog機能の実装
- **Phase 4: Real Experience**: Coming Soonページの実装

### **データベース拡張**
- **ゲーミフィケーション**: `profiles`テーブル拡張、`missions`/`user_missions`テーブル作成
- **エンゲージメント**: `products`/`fan_photos`テーブル作成、RLSポリシー設定
- **Gallery拡張**: `gallery_events`/`fan_photo_likes`テーブル作成、`fan_photo_like_counts`ビュー作成
  - イベント管理機能、投稿上限トリガー、アクティブイベント制約トリガー

### **新規コンポーネント・Hooks**
- **Marketing**: `RankBadge`, `MissionCard`, `ComingSoonBadge`
- **Shop**: `ProductCard`
- **Gallery**: `PhotoGrid`, `UploadModal`, `AdminEventPanel`, `AdminReviewPanel`
- **Blog**: `NarratorTemplate`
- **Hooks**: `useGamification`, `useShop`, `useGallery`（いいね機能・イベント管理機能を含む）

## 🔄 最近の変更（2025-11-02）

- **HomePageにお知らせセクションを追加**: Supabaseから最新情報を取得し、モダンなカード形式で表示
- **HomePageデザイン改善**: Glassmorphism強化、グラデーションボーダー、マイクロインタラクション、スペーシング統一
- **お知らせシステム実装**: `useAnnouncements`フックと`AnnouncementCard`コンポーネントを新規作成
- **RLSポリシー追加**: 全ユーザー（匿名含む）がお知らせを読み取れるように設定
