...
**作成日**: 2026-04-10  
**スナップショット更新**: 2026-04-12（MCP `execute_sql` 再取得）  
**想定読者**: コンテンツ執筆者、プロダクトオーナー、AI アシスタント  
**関連**: [08_Syllabus_Management_Guide.md](08_Syllabus_Management_Guide.md)（分類の正本）、[06_記事作成ロードマップ.md](06_記事作成ロードマップ.md)（Phase 計画）、[07_PPL_Master_Syllabus.md](07_PPL_Master_Syllabus.md)（PPL 進捗）
...
| 指標 | 値 | 備考 |
|------|-----|------|
| **verified 設問クラスタ数** | **224** | `DISTINCT (main_subject, sub_subject)` |
| **`learning_contents` 総行数** | **87** | 2026-04-12 時点 |
| **`learning_contents`・category 内訳** | CPL学科 **49** / PPL **10** / 他 | `GROUP BY category` |
| **`learning_test_mapping` がある記事** | **42** | `learning_content_id` 単位・行数とも 42（2026-04-12 時点） |
| **リポジトリ `src/content/lessons/*.mdx`** | **66** | 2026-04-12 時点（ファイル数） |
...
## 2. `learning_test_mapping` が付いている記事（42件）

クイズ結果の **ReviewContentLink** がマッピング優先で参照しうる `learning_content_id`（2026-04-12 時点・`ORDER BY learning_content_id`）:
...
**航空気象 3.3.x**（マッピングあり）: `3.3.1_StandardAtmosphere` 〜 `3.3.7_WesterliesAndWind`、`3.3.9_WeatherHazards`、`3.3.10_CloudTypes`、`3.3.11_VisibilityAndFog`  
→ **`3.3.8_ICAOStandardAtmosphere`・`3.3.12_Turbulence` は MDX のみでマッピング行なし**（拡張余地）

**空中航法 3.4.x**（マッピングあり）: `3.4.4_FlightPlanning`、`3.4.7_DeadReckoningAdvanced`  
→ **`3.4.1`〜`3.4.3`・`3.4.5`・`3.4.6` は MDX のみでマッピング行なし**（拡張余地）

**航空通信 3.5.x**（マッピングあり）: `3.5.1_AirTrafficServices` 〜 `3.5.4_EmergencyProcedures`  
→ **`3.5.5_ATCPhraseology` は MDX のみでマッピング行なし**（拡張余地）
...
## 3. リポジトリに **ある** MDX（`src/content/lessons`・66）
...
**空中航法（CPL 3.4 スタブ）**: `3.4.1_DeadReckoning` 〜 `3.4.7_DeadReckoningAdvanced`  

**航空通信（CPL 3.5 スタブ）**: `3.5.1_AirTrafficServices` 〜 `3.5.5_ATCPhraseology`  

**ハブ・気象入口**: `CPL-Hub-Meteorology`, `CPL-Hub-Navigation`, `CPL-Hub-Communication`, `weather_basics`  

**PPL（航空工学系・17本）**:  
`PPL-1-1-1_TemperatureBasics`, `PPL-1-1-2_AirspeedBasics`, `PPL-1-1-3_BernoulliPrinciple`, `PPL-1-1-4_DragBasics`, `PPL-1-1-5_AxesStability`, `PPL-1-1-6_StallSpin`, `PPL-1-1-7_VnDiagram`, `PPL-1-1-8_PropellerBasics`, `PPL-1-1-9_FlightPerformance`, `PPL-1-1-10_TakeoffLandingPerformance`, `PPL-1-2-1_GyroBasics`, `PPL-1-2-2_PitotStatic`, `PPL-1-2-3_MagneticCompass`, `PPL-1-2-4_ReciprocatingEngine`, `PPL-1-2-5_CarburetorMixture`, `PPL-1-2-6_IgnitionMagneto`, `PPL-1-2-7_LubricationCooling`
...
| **粒度のある気象記事 `3.3.*`** | **2026-04-10** に `3.3.1`〜`3.3.12` スタブ MDX を追加。ハブ `CPL-Hub-Meteorology` と併用。**2026-04-12** 時点で `learning_test_mapping` は 10/12 本（`3.3.8`・`3.3.12` は未マッピング）。 |
| **航法・通信の一部 `3.4.*` / `3.5.5`** | MDX はあるが **マッピングが未整備**の id あり（§2 参照）。 |
...
### Tier B — [06](06_記事作成ロードマップ.md) Phase1+2：**スタブ MDX は配置済み**（本文・マッピング精緻化は継続）
...
### Tier C — PPL シラバス残り（[07](07_PPL_Master_Syllabus.md)）
