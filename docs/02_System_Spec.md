...
**URL 正本**（[`testHubFilters.ts`](../src/pages/test/testHubFilters.ts)）: `tab`, `sort`（`priority` \| `syllabus`）、既存 `subject`, `sub`, `count`, `mode`, `contentId`, `exam`。レガシー（`tab` なし + `mode`/`subject` 等）はマウント時に canonical へ `replace`。**`setSearchParams` は `areTestHubSearchParamsEqual` で実際に変わるときだけ実行**（診断タブで `count=0` を URL に書くと parse 側が 10 に正規化し `history.replaceState` がループする不具合を防止）。部分更新は `applyPartialTestHubState`。
...
| イベント | タイミング | 主なパラメータ |
|----------|------------|----------------|
| `quiz_hub_view` | タブ・試験レベル・記事連動 ID の変化 | `tab`, `exam`, `content_id` |
| `quiz_filter_open` | フィルタドロワー開 | `tab`, `exam` |
| `quiz_session_start` | 問題取得成功・セッション開始 | `tab`, `mode`, `count`, `subject`, `content_id`, `exam` |
| `quiz_start` | 同上（探索用エイリアス・同一 payload） | 同上 |
| `quiz_session_complete` | 提出完了 | `score_pct`, `count`, `mode` |
...
**デプロイ後の JS チャンク不整合**（Sentry: `Importing a module script failed` 等）: [`lazyWithRetry`](../src/utils/lazyWithRetry.ts)（主要 lazy ルート）、[`chunkLoadRecovery`](../src/utils/chunkLoadRecovery.ts)（1 セッション 1 回の自動リロード）、[`EnhancedErrorBoundary`](../src/components/ui/EnhancedErrorBoundary.tsx)・[`MDXLoader`](../src/components/mdx/MDXLoader.tsx) の再読み込み UI、[`public/sw.js`](../public/sw.js) の **script/style は network-first**（キャッシュ名 `flight-academy-shell-v2`）。
...
- **レイアウト（2026-06 改訂）**: モバイルは `/profile` で 4 セクション一覧 → `?tab=` でドリルダウン + 「設定に戻る」。デスクトップ（`md+`）は左 sticky `ProfileHubSidebar` + 右コンテンツ。二段 pill ナビは廃止
- **4 セクション**: `profile`（公開プロフィール + ソーシャル）、`learning`（cohort + 週次バッジ）、`privacy`（通知 auto-save + ランキング）、`account`（メール + パスワード + OAuth 一覧 + MFA 入口 + アカウント削除）
- **完成度**: [`ProfileCompletionStrip`](../src/pages/profile/components/ProfileCompletionStrip.tsx) — username 20 / avatar 25 / bio 20 / social 10 / website 10 / 通知設定 15（`user_notification_settings` 保存済み）。cohort は `/welcome` 完結のため含めない
- **URL 同期・後方互換**: `?tab=` で深リンク。レガシー `leaderboard`→`privacy`、`cohort`→`learning`、`social`→`profile`、`security`→`account`。`?panel=` は読取互換のみ（無視）
- **未保存ガード**: 明示 Save フォーム（profile / learning / privacy のランキング）に `useUnsavedFormGuard` + モバイル sticky 保存バー。通知は debounce auto-save（Save ボタンなし）
- **アカウント削除**: `POST /api/account/delete`（JWT + 確認フレーズ + 必要時パスワード再認証）→ `auth.admin.deleteUser`
- **二要素認証（MFA）**: Supabase TOTP。`profiles.mfa_required_at_login`（デフォルト **false**・opt-in）でログイン時 MFA を制御。**パスワード変更・アカウント削除**はログイン MFA が OFF でも、verified 因子がある場合は 6 桁コード必須（[`mfaAuth.ts`](../src/auth/mfaAuth.ts)）。ログイン直後・保護ルートでは AAL1 のままなら `/auth?mfa=required` でコード入力。**リカバリーコード**（10 件・使い捨て）は有効化時または Profile 再発行時に一度表示（[`mfaRecoveryCodesCore.ts`](../api/_lib/mfaRecoveryCodesCore.ts)）。API は **`api/mfa-recovery-codes.ts`**（`/api/mfa-recovery-codes?action={generate|consume|status|clear}`）。`generate` / `clear` は **AAL2 必須** — サーバーは verified JWT の **`aal` クレーム**で判定（ステートレス client の `getAuthenticatorAssuranceLevel()` は使用不可）。**本番確認済**（2026-06-21: Profile 再発行で 10 件表示）。ログイン MFA 画面から消費すると 2FA 解除＋ログイン完了。iPhone は「パスワード」App、Android は Authenticator 等（メール非送付）
- **管理機能**: Profile には置かない。ヘッダー **ADMIN** + `/admin` Hub + UserMenu「管理 Hub」のみ
