...
- **二要素認証（MFA）**: Supabase TOTP。`profiles.mfa_required_at_login`（デフォルト **true**）でログイン時 MFA の ON/OFF を制御（2FA 有効時のみ Profile で変更可）。**パスワード変更・アカウント削除**はログイン MFA が OFF でも、verified 因子がある場合は 6 桁コード必須（[`mfaAuth.ts`](../src/auth/mfaAuth.ts)）。ログイン直後・保護ルートでは AAL1 のままなら `/auth?mfa=required` でコード入力。**リカバリーコード**（10 件・使い捨て）は有効化時に一度表示（[`mfaRecoveryCodesCore.ts`](../api/_lib/mfaRecoveryCodesCore.ts)、`POST /api/account/mfa-recovery-codes/*`）。ログイン MFA 画面から消費すると 2FA 解除＋ログイン完了。iPhone は「パスワード」App、Android は Authenticator 等（メール非送付）
