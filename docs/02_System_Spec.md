...
- `ReadingProgressBar`: 記事詳細で `learning_progress` へスクロール進捗を保存（表示は行わない。目次側でメーター表示）
- `MDXLoader`: 動的記事読み込み
...
- **対象**: 自己啓発・思考法系 MDX 28 本（ファイル名＝`learning_contents.id`＝URL の `contentId`）。
...
- **データ再読み込み機能**: `refreshProgress` により、Supabase から最新の進捗を再取得可能（`/articles` 復帰時など）
...
#### **記事ステータス判定システム（2026年1月実装、2026年4月更新）**
...
### **記事の読み順（シリーズメタ・2026年4月更新）**

- **閲覧制限**: なし。全公開記事は URL 直アクセス・一覧からいつでも開ける（クイズ結果の推奨記事とも矛盾しない）。
- **執筆・ナビ用**: MDX の `meta.series` / `meta.order` は推奨読み順と一覧の「次に読む」ハイライトに使用。記事末尾の PrevNext は Supabase `learning_contents.order_index`（カテゴリ内）で決まる。
- **未ログイン向け登録促進**: [ReviewContentLink](src/pages/articles/components/learning/ReviewContentLink.tsx) で推奨記事リストの**直上**に、ゲストのみ短い登録メリットと `/auth` への CTA を表示（記事本文はブロックしない）。

### **PPL 記事と CPL 記事（戦略との整合・2026年4月）**
...
**試験問題の正本**は **`unified_cpl_questions`**。アプリ内ギャラリー・ショップは撤去（`/gallery`・`/shop` ルートは **削除済み**。外部ギャラリーはリンク集ページの URL のみ）。
...
- **パイロット例**: [docs/db/CPL_KPI_and_Database_Operations.md](db/CPL_KPI_and_Database_Operations.md)
...
- **継続日数（ダッシュボード表示）**: `streak_records` は日次で `src/utils/streak.ts` の `updateStreak` で更新する。クライアントは記事の初回 95% 到達・完了、レッスン完了（`useLearningProgress.markAsCompleted`）、クイズ結果保存成功後に `syncStreakToUserLearningProfile` を呼び出し、その戻り値で `user_learning_profiles` の `current_streak_days` / `longest_streak_days` を upsert する。`fetchDashboardMetrics` の `getStreakDays` はこの `user_learning_profiles` を参照する。
...
- **学習記事エンゲージメント**（`learning_content_likes` / `learning_content_views`）: いいねは **認証ユーザー** が **本人の `user_id` の行のみ** INSERT/DELETE。閲覧記録は **認証** 時は `user_id = auth.uid()`、**匿名（anon）** 時は `user_id IS NULL` かつ **`session_id` 必須**、いずれも **`content_id` が `learning_contents.id` に存在**すること。広い `WITH CHECK (true)` は採用しない（Supabase linter 0024 対策）。適用用 SQL: [scripts/database/20260414_supabase_security_linter_fixes.sql](../scripts/database/20260414_supabase_security_linter_fixes.sql)
- 公開情報は読み取り可能
...
- **イタリア域の AIP 相当データ**（空港・空域・NAVAID 等）を Planning 地図に参照レイヤーとして追加する案。**現状は保留**。データソース、表示・再配布の条件、既存の日本域レイヤーおよび SWIM NOTAM との役割分担は着手時に調査する。優先度と経緯の正本は [01_Current_Status_and_Roadmap.md](01_Current_Status_and_Roadmap.md)「将来保留バックログ」。
...
### **マーケティングレイアウト・レガシーブランド色（移行前の参照）**

Flight Academy へのブランド移行は [00](00_Flight_Academy_Strategy.md) §7・Phase C。以下は**現行 UI の説明**として残す。

#### **Phase 1: Brand Foundation（実装済み）**
...
- **シリーズ順次ロックの廃止**（2026年4月）: 全記事常時閲覧。ゲスト向け登録 CTA は `ReviewContentLink` 先頭。
...
- HUDテーマ削除、マーケティング用ダーク×黄テーマへ統一（履歴）
