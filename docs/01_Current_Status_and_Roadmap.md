...
**最終更新**: 2026年9月20日（第2波: ポインタ重複を 00/06/08 へ。May 節を Closed_Sprints ポインタへ）  
**バージョン**: Roadmap v4.0.50
...
- **記録**: [Closed_Sprints.md](Closed_Sprints.md) と [05](05_Content_Pipeline.md) **W20〜W22**。カバレッジ運用は [06](06_Long_Term_Execution.md) §1。品質監査メモ: [`artifacts/accessibility_audit_memo_2026-05-06.md`](../artifacts/accessibility_audit_memo_2026-05-06.md)。

### 2026年6月期スプリント（完了）

- **記録**: [Closed_Sprints.md](Closed_Sprints.md) と [05](05_Content_Pipeline.md) **W23〜W26**。PPL Subject 2 Phase 1 **12/12**。

### 2026年7月期スプリント（完了）

- **記録**: [Closed_Sprints.md](Closed_Sprints.md) と [05](05_Content_Pipeline.md) **W27〜W30**。Subject 3/4 Phase 1 完走。Callout 法規 8/8。
...
- **記録**: [Closed_Sprints.md](Closed_Sprints.md) と [05](05_Content_Pipeline.md) **W31〜W35**。A2-a 実装（科目 default 5問）。coverage **21.17%**。CBT 暫定束ねは 05 の記録（当該 SQL は git に無い）。

### 2026年9月期スプリント（実行中 — 2026-09-20 時点）

月次計画ファイルは置かない。週次は [05](05_Content_Pipeline.md)。畳み込み: [Closed_Sprints.md](Closed_Sprints.md)。

| 優先 | タスク | 2026-09-20 の事実 |
|------|--------|-------------------|
| **1** | **Articles 週3本** | **守れている**（W38 CP 4-2/5-1/5-2、W39 CP 5-3/5-4/5-5 を schedule）。唯一の成長習慣。 |
| **2** | **A2-a 計測** | **判定不能**。[ops/Weekly_Telemetry_Review.md](ops/Weekly_Telemetry_Review.md) W37: 週間 users 一桁、`quiz_*` **0**。2週連続改善は測れない。 |
| **3** | **CBT Phase B** | **未着手**。方針メモファイルは git に一度も無い。 |
| **4** | **PPL-2-3-4** | **未執筆**。骨子のみ。 |
| **5** | **PPL-2-3-3** | 01/05 は W34 完了と書くが、**MDX も SQL も git に無い**。残は骨子のみ。 |
| **6** | C-1〜C-5 | **未承認のまま着手しない**。 |
| **7** | 10月計画ファイル | **作らない**。 |

**PPL**: Subject 2/3/4 Phase 1 の MDX は git にある。`PPL-2-3-3` を「済」と数えない。
...
| **PPL 記事**                        | MDX: Subject 1〜5 Phase 1 は git にある。Subject 2 Phase 1 は `PPL-2-1-1`〜`PPL-2-3-2`。**`PPL-2-3-3` / `2-3-4` は骨子のみ**（MDX なし） | **継続** — 09 の優先は週3本ドリップ。Phase 2 気象は余力 |
...
| B-7（任意） | 持続可能性メモ              | **統合済**: [00 §3.1](00_Flight_Academy_Strategy.md)（外部 API 運用）。Notion 等の運用と二重管理にしない           | 低   |
...
- テストカバレッジ: **`src` 実効 Statements** の主目標は **Phase C（2026-06〜07）で 15% 以上**（ストレッチ **18%**・任意）。Phase B 終盤は B-4 で段階拡充。**履歴スナップショット**は [06](06_Long_Term_Execution.md) §1.1（5月 13.99% → 6月 18.07% → 7–8月 約 21%）。最新の追記は本書更新履歴 / [Closed_Sprints.md](Closed_Sprints.md)
...
| 項目 | 目的・範囲 | 備考 |
|:-----|:-----------|:-----|
| **Quiz Hub UX（科目完走・流入）試験明け** | 科目タブ完走率・Direct 依存の改善（UI 含む）。C-1〜C-5 は承認後のみ | 記録: [Closed_Sprints.md](Closed_Sprints.md) |
...
| 日付         | 更新内容                                                                                                                                                                                                                  |
| ---------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-09-20 | **v4.0.49 / docs スリム**: 月次計画 7 本を [Closed_Sprints.md](Closed_Sprints.md) へ畳み込み。実装済み Gemini ブリーフ削除。9月の A2-a / CBT Phase B / PPL-2-3-4 は未完と明記。`PPL-2-3-3` の MDX は git に無い。 |
| 2026-08-24 | **v4.0.48 / ログイン誤リセットの修正**: リカバリー判定を `type=recovery` / `/auth/recovery` / `mode=recovery` に限定（implicit flow の `access_token` ハッシュだけでは判定しない）。フラグは TTL 15 分・非 recovery コールバックと `signIn` 成功でクリア。`/auth/recovery` に再送・中断導線、ハッシュ側エラーも表示。Zustand 永続化から `session` 除外、`shouldPromptLoginMfa` を 60 秒キャッシュ、動作しない `bypassEmailVerification` を撤去。[02](02_System_Spec.md)。 |
...
| 2026-08-12 | **9月計画起票**: [Closed_Sprints.md](Closed_Sprints.md) — A2-a 計測 W36–W39、CBT Phase B、W36–W40 週次。 |
...
| 2026-08-12 | **v4.0.43 / W34**: CBT 気象+航法 **各18**→ハブ、**PPL-2-3-3** + mapping **12**、[Action2/3 方針](Closed_Sprints.md)。MCP: 未マッピング **67**、mapping **145**、PPL **64/150**。 |
| 2026-08-12 | **v4.0.42 / W33 CBT + Articles**: [CBT 方針メモ](Closed_Sprints.md) + 航空通信 CBT **11**→`CPL-Hub-Communication`。MCP: 未マッピング総 **103**（CBT **93** + レガシー **10**）、mapping **141 行**、`learning_contents` **152**。Articles「訓練の当たり前」W32/W33 ドリップ進行。[August](Closed_Sprints.md)・[05](05_Content_Pipeline.md) 再整理。 |
| 2026-07-25 | **v4.0.41 / W30 七月末ゲート + Phase C クローズ**: Tier B 着陸装置 **2 問**→`engineering_basics`。MCP: 未マッピング総 **112**（CBT例題除外 **10**・`CBT例題/未分類` **102**＝MLIT 例題取込）、mapping **140 行**。`src` Statements **21.18%**（7509/35445）。`test:run` **481** 緑。PPL **63/150**・Subject 3/4 Phase 1 完走。C-1〜C-5 は未着手継続。[August_2026_Implementation_Plan](Closed_Sprints.md) 起票。 |
...
| 2026-06-30 | **v4.0.34 / 六月スプリント完走 + 7月計画**: W26 ゲート — verified 未マッピング **23**、mapping **106 行 / 95 記事**。PPL Subject 2 Phase 1 **12/12**（`PPL-2-2-1`〜`2-3-2`）。累計 **46/150**。CPL Callout **`3.3.3/4/7/9/12`**。Tier B 航法計器→`3.4.2`。`src` **18.07%**。[July_2026_Implementation_Plan](Closed_Sprints.md) · [July_2026_Content_Sprint](Closed_Sprints.md) 初版。 |
...
| 2026-06-06 | **v4.0.22 / 6月期ゲート・Quiz Hub**: Lane A **PR-Q0〜Q5** — `/test` Quiz Hub（`testHubFilters`・UI 分割・GA4 5 イベント・学習ループ）。`npm run test:coverage` → **`src` Statements 18.07%**（5302/29341）。[06 §1](06_Long_Term_Execution.md)、[02 §/test](02_System_Spec.md)。Lane B: W24 マッピング（空力基礎→`3.2.7`、未マッピング **47→36**）、PPL Callout、`testHubFilters.test.ts`（14）。A11y: [`accessibility_audit_memo_2026-06-06.md`](../artifacts/accessibility_audit_memo_2026-06-06.md)。 |
...
| 2026-05-09 | **v4.0.20 / May 後半 W21→W22 ドキュメント整備**: **W21 完了** — [`3.2.8`](../src/content/lessons/3.2.8_PowerAndPerformance.mdx)、[`PPL-1-1-9`](../src/content/lessons/PPL-1-1-9_FlightPerformance.mdx)。[May](Closed_Sprints.md) §6・§2、[05](05_Content_Pipeline.md) W21/W22、[May 後半アウトライン](Closed_Sprints.md)、[content_outlines/README.md](content_outlines/README.md) を整合。**W22 実行素案** [content_outlines/README.md](content_outlines/README.md) 追加、**W21** `content_outlines/W21_2026/` 撤去。**[docs/README](README.md)** 索引 v4.48。 |
| 2026-05-08 | **v4.0.19 / May 後半 W20→W21 ドキュメント整備**: W20 で実装済みの **コンテンツ**（[`3.2.7`](../src/content/lessons/3.2.7_LiftAndDrag.mdx)、[`PPL-1-1-3`/`4`](../src/content/lessons/PPL-1-1-3_BernoulliPrinciple.mdx)）と [May](Closed_Sprints.md) §6 完了ログ、[05](05_Content_Pipeline.md) W20/W21 行、[May 後半アウトライン](Closed_Sprints.md) を整合。**W21 実行用ワーク**は `content_outlines/W21_2026/`（**2026-05-09 実装後に削除**）。W20 用 Gemini 素案ファイル削除。 |
| 2026-05-07 | **v4.0.18 / 5月後半コンテンツ拡充メモ**: [Closed_Sprints.md](Closed_Sprints.md)（W20〜W22・CPL/PPL 対応）、[Closed_Sprints.md](Closed_Sprints.md)、[templates/External_LLM_Article_Brief.md](templates/External_LLM_Article_Brief.md)。`---## 現状評価` の改行欠落を修正。[docs/README](README.md) に索引。 |
| 2026-05-07 | **v4.0.17 / 6月期計画書**: [Closed_Sprints.md](Closed_Sprints.md) を追加。**2026-W23〜W26** の週次ゴール・マッピング 1 サイクル・Phase C 準備と C-1〜5 の境界を定義。本書に **「2026年6月期スプリント」** 参照を追加。[docs/README](README.md) に索引。**[06 §1](06_Long_Term_Execution.md)** に **「2026-06 末」行**を 6月末に追記する前提。 |
| 2026-05-06 | **v4.0.16 / 5月期残タスク一括（計画）**: CPL→PPL 復習 Callout を **[3.2.10](../src/content/lessons/3.2.10_WeightAndBalance.mdx) / [3.2.12](../src/content/lessons/3.2.12_EngineSystems.mdx) / [3.4.4](../src/content/lessons/3.4.4_FlightPlanning.mdx)** に追加（案 A：最低本数達成）。**[05](05_Content_Pipeline.md)**: Phase 2 週次 **W19〜W22** と暫定 KPI 段落。**[14](Article_Coverage_Backlog.md)**: MCP `execute_sql` で §1 指標を再検証し前回値と一致（新規 SQL なし）。**B-4**: [`aviationWeatherApiCore.test.ts`](../src/__tests__/api/aviationWeatherApiCore.test.ts)、[`openskyStatesCore.test.ts`](../src/__tests__/api/openskyStatesCore.test.ts)、[`weatherApiCore.test.ts`](../src/__tests__/api/weatherApiCore.test.ts)。**B-6**: `createBrowserClient` シングルトンを静的確認。**B-7**: [00 §3.1](00_Flight_Academy_Strategy.md)。**Phase C 準備**: [`artifacts/accessibility_audit_memo_2026-05-06.md`](../artifacts/accessibility_audit_memo_2026-05-06.md)。`npm run test:coverage` の **`src` 実効 Statements 約 13.99%**（`api/` は集計外のため増分なし）。[06](06_Long_Term_Execution.md) 月末表を記入済み。 |
| 2026-05-06 | **v4.0.15 / B-4 残タスク一括**: [`useArticleProgress.hook.test.tsx`](../src/__tests__/hooks/useArticleProgress.hook.test.tsx)、[`mdxContentParsing.ts`](../src/utils/mdxContentParsing.ts)+[`mdxContentParsing.test.ts`](../src/__tests__/utils/mdxContentParsing.test.ts)、[`structuredData.test.ts`](../src/__tests__/utils/structuredData.test.ts)、[`swimNotamGeometry.test.ts`](../src/__tests__/api/swimNotamGeometry.test.ts)、[`planDocument.test.ts`](../src/__tests__/utils/planDocument.test.ts) を追加。技術的負債表の **`src` 実効 Statements を約 13.99%** に更新。[06](06_Long_Term_Execution.md) 月末メモ、[docs/README](README.md) カバレッジ行と整合。`vitest.config.ts` 閾値は据え置き。 |
| 2026-05-06 | **v4.0.14 / カバレッジ KPI 再定義・再計測・Wave A**: `src` 実効 Statements の**主目標を Phase B 一発 30% から Phase C 15%**（ストレッチ 18%）へ変更。成功指標サマリー・Phase C KPI・Phase D の注記・リスク表を追随。**2026-05-06** `npm run test:coverage` で **約 11.89%**（`FlightAcademyTsx/src/` のみ集計）を技術的負債表・[06](06_Long_Term_Execution.md)・[docs/README](README.md)・[06](06_Long_Term_Execution.md) §1.1 に反映。[00](00_Flight_Academy_Strategy.md) 弱みの記述を整合。`vitest.config.ts` の閾値は据え置き。[`taskGenerator.test.ts`](../src/__tests__/utils/taskGenerator.test.ts) を追加。 |
| 2026-05-06 | **v4.0.13 / パイプライン・マッピング tier2・品質メモ**: [05](05_Content_Pipeline.md) Phase 2 週次着手記録。[14](Article_Coverage_Backlog.md) に [`20260506_learning_test_mapping_unmapped_tier2.sql`](../scripts/database/20260506_learning_test_mapping_unmapped_tier2.sql)（verified 未マッピング **69**・行 **58**）。[`useArticleProgress.test.ts`](../src/__tests__/hooks/useArticleProgress.test.ts) 拡張。[06](06_Long_Term_Execution.md) 月末表、[06](06_Long_Term_Execution.md) §1.4（`tsc -b`）。 |
...
| 2026-05-05 | **v4.0.10 / May sprint**: 「2026年5月期スプリント」節追加。Phase 2 級の **3.5.4** 緊急通信レッスンの本文化与 DB メタ同期 SQL、[06_Long_Term_Execution.md](06_Long_Term_Execution.md)、監査／再試行用 `20260505_*.sql`、[04_Operations_Guide.md](04_Operations_Guide.md) GA4 ログ表の運用手順明示。`calculateLearningStats` テスト拡張。詳細は同節参照。 |
