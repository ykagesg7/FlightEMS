...
週次メール案内（X 保留時）: cron `article-weekly-digest`（日曜 17:00 JST／来週予告＋今週リマインド、月曜 07:00 JST／抜け時キャッチアップ）。一時的に明示メールOFF以外へブロードキャスト。詳細は [05_Content_Pipeline.md](../05_Content_Pipeline.md) §1。
