...
### 全体進捗
- **全体**: 3/150 (2%)
- **Phase 1**: 3/45 (6.7%) - 高頻出・重要トピック
- **Phase 2**: 0/60 (0%) - 中頻出トピック
...
|------|------|------|------|---------|---------|---------|
| **Subject 1: 航空工学** | 45 | 3 | 6.7% | 3/15 | 0/20 | 0/10 |
...
- [ ] **Phase 1** 安定性: 三軸（縦・横・方向）の安定、重心位置の許容限界、上反角効果
- [ ] **Phase 1** 操縦性: 舵の効き、トリム、三舵（エルロン・エレベータ・ラダー）の働き
- [ ] **Phase 2** プロペラ: 翼素理論、可変ピッチ・プロペラの仕組み
- [ ] **Phase 1** 失速の種類: 概要、失速警報装置

#### 1-1-2. 性能と耐空性
- [ ] **Phase 1** 飛行性能: 水平飛行、失速速度、上昇・滑空・旋回性能
- [ ] **Phase 2** 設計強度: 制限荷重（リミットロード）、安全率、V-n線図
...
- [ ] **Phase 2** 遠隔表示計器: ジャイロの原理、ジャイロスコープの剛性と歳差
- [ ] **Phase 1** 姿勢表示計器: 旋回計・旋回釣合計、定針儀（DG）、姿勢儀（AI）
- [ ] **Phase 1** エア・データ表示計器: ピトー・静圧系統、高度計（ALT）、対気速度計（ASI）、昇降計（VSI）
- [ ] **Phase 1** 航法計器: 磁気コンパス（誤差含む）、VOR指示器
- [ ] **Phase 2** レーダー: トランスポンダー（SSR）の概要

#### 1-2-3. 動力装置（ピストン・エンジン）
- [ ] **Phase 1** 作動原理: 4サイクルエンジンの基本行程
...
- [ ] **Phase 1** 混合気供給系統: キャブレター、インジェクション、アイシング（気化器凍結）
- [ ] **Phase 1** 点火・始動系統: マグネトー（点火磁石発電機）の信頼性
- [ ] **Phase 2** 潤滑系統: オイルの役割と循環システム
- [ ] **Phase 2** 冷却系統: 空冷エンジンの運用、ショッククーリング
...
| 2026-01 | Phase 1記事3件作成完了（温度換算、対気速度、ベルヌーイの定理）、KaTeX数式記法サポート追加 | System |
