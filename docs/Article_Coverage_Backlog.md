...
**作成日**: 2026-04-10  
**スナップショット更新**: 2026-07-10（W29 完走 — mapping **122 行**・verified 未マッピング **12**）
...
**MCP 再取得（W29 七月第 3 サイクル）**: 2026-07-10 — Tier B **航法計器/レーダー 2 問**（ATC トランスポンダ）→ `3.5.3_RadioCommunication`（[`20260710_learning_test_mapping_transponder_w29.sql`](../scripts/database/20260710_learning_test_mapping_transponder_w29.sql)）。法規 Callout **`3.1.6`** ← `PPL-5-4-7`。
...
**MCP 再取得（W28 七月第 2 サイクル）**: 2026-07-09 — **PPL Subject 3 Phase 1 完走**（`PPL-3-1-3`〜`PPL-3-3-2` 深文化 **7 本** + mapping **+9 行**）。SQL: [`20260707_*`](../scripts/database/INDEX.md)・[`20260708_*`](../scripts/database/INDEX.md)・[`20260709_*`](../scripts/database/INDEX.md)。
**MCP 再取得（W30 七月第 3 サイクル）**: 2026-07-08 — **PPL-3-1-3**〜**PPL-3-2-2** 深文化 **5 本** + mapping（[`20260707_*`](../scripts/database/INDEX.md)・[`20260708_*`](../scripts/database/INDEX.md)）。
...
**登録ログ（未再集計）**: **2026-06-28** — **`PPL-1-2-1`〜`7`** を `learning_contents` に登録（[`20260628_learning_contents_ppl_121_127.sql`](../scripts/database/20260628_learning_contents_ppl_121_127.sql)）。続けて [`20260627_backfill_ppl_engineering_test_mapping.sql`](../scripts/database/20260627_backfill_ppl_engineering_test_mapping.sql) で **`learning_test_mapping` 7 行**追補。**2026-06-27** — **`learning_test_mapping` PPL Subject 2 Phase 1**（`PPL-2-1-1`〜`4`、大気の基礎/大気・温度・気圧・水分）を MCP `execute_sql` 済。SQL: [`20260627_learning_test_mapping_ppl_subject2_201_204.sql`](../scripts/database/20260627_learning_test_mapping_ppl_subject2_201_204.sql)。**同日** — PPL Subject 1 工学 **`PPL-1-1-2`〜`1-1-10`** を [`20260627_backfill_ppl_engineering_test_mapping.sql`](../scripts/database/20260627_backfill_ppl_engineering_test_mapping.sql) で本番適用（`PPL-1-2-1`〜`7` は `learning_contents` 未登録のため 0 行）。CPL 気象 **`3.3.1`/`2`/`5`/`6`/`8`/`10`/`11`** に PPL-2-* 復習 Callout を MDX 更新。**2026-06-27** — PPL Subject **2（航空気象）** Phase 1 **`PPL-2-1-4`** を `learning_contents` に追加（`sub_category`: **航空気象**、`order_index`: **204**、`is_published`: true）。冪等 SQL: [`20260627_learning_contents_ppl_subject2_204.sql`](../scripts/database/20260627_learning_contents_ppl_subject2_204.sql)（MCP `execute_sql` 済）。**2026-06-24〜25** — **`PPL-2-1-2`・`PPL-2-1-3`**（`order_index` **202〜203**）。**2026-06-24** — **`PPL-2-1-1`**（`order_index` **201**）。**2026-05-12** — PPL Master Subject **5（航空法規）** の本文化済み **4 件** を `learning_contents` に追加（`sub_category`: **航空法規**、`order_index`: **501〜504**、`is_published`: true）。冪等 SQL: [`20260512_learning_contents_ppl_aviation_law_four.sql`](../scripts/database/20260512_learning_contents_ppl_aviation_law_four.sql)。**同一日追補（未再集計）**: **`PPL-5-3-1`〜`PPL-5-3-3`**（技能証・身体検査・航空英語、`order_index` **505〜507**）を [`20260512_learning_contents_ppl_subject5_505_507.sql`](../scripts/database/20260512_learning_contents_ppl_subject5_505_507.sql) で MCP `execute_sql` 済み。**続けて同一日追補**: **`PPL-5-4-1`〜`PPL-5-4-7`**（空域〜最低高度入門、`order_index` **508〜514**）を [`20260512_learning_contents_ppl_subject5_508_514.sql`](../scripts/database/20260512_learning_contents_ppl_subject5_508_514.sql) で MCP `execute_sql` 済み。**§1 の `learning_contents` 総行数 90／PPL 13 はスナップショット由来のため未更新**（再取得は §5）。
...
| **`learning_contents` 総行数** | **106** | 2026-07-09（`is_published` — PPL **55** + CPL **49** + 操縦 **2**） |
| **`learning_contents`・category 内訳** | CPL学科 **49** / PPL **55**（工学 **20** + 法規 **14** + 気象 **12** + 航法 **9**） / 操縦 **2** | Subject 3 **`PPL-3-1-1`〜`3-3-2`** Phase 1 **完走** |
| **PPL 記事のマッピング** | **55 / 55** 登録済 | 法規 **14**・気象 **12**・工学 **20**・航法 **9**（`3-3-1` は 5-4-1 に分担） |
| **`learning_test_mapping` がある記事** | **103** | 2026-07-09 MCP |
| **`learning_test_mapping` 行数** | **122** | 2026-07-10（W29 Tier B **+2** 行） |
| **verified 未マッピング設問** | **12** | 2026-07-10（**17→12**） |
| **リポジトリ `src/content/lessons/*.mdx`** | **119** | 2026-07-09（PPL **63** MDX — 公開 **55** + Subject 4 スタブ **8**） |
...
### 1.1 PPL 系列 — Callout / `learning_test_mapping` 監査（2026-06-27）
...
**注意**: 下表の **件数**は **2026-07-10** W29 MCP 再取得値。着氷・トランスポンダ・電波の伝播クラスタは **適用済**。次候補: **着陸装置/降着装置の形式 2**。

2026-07-06 時点で、**未マッピング設問数が多い** `sub_subject` の例:
...
| 2026-07-10 | **W29 Tier B + Callout**: 航法計器/レーダー **2 問**（ATC トランスポンダ）→ `3.5.3_RadioCommunication`（[`20260710_learning_test_mapping_transponder_w29.sql`](../scripts/database/20260710_learning_test_mapping_transponder_w29.sql)）。法規 Callout **`3.1.6`** ← `PPL-5-4-7`（**6/8**）。MCP: mapping **121 行 / 103 記事**・未マッピング **15**。 |
| 2026-07-09 | **PPL Subject 3 Phase 1 完走**: [`PPL-3-3-1`](../src/content/lessons/PPL-3-3-1_VfrOperationsAndAirspace.mdx)・[`PPL-3-3-2`](../src/content/lessons/PPL-3-3-2_SpatialDisorientationBasics.mdx) 深文化 + mapping **22 問**（`3-3-2`）。**9/9 本**完走。MCP: `learning_contents` **106**・mapping **120 行 / 103 記事**・未マッピング **17**。 |
