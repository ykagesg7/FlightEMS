...
- **推奨記事表示**: [ReviewContentLink](src/pages/articles/components/learning/ReviewContentLink.tsx) は、解答後の推奨で **両列の `overlaps`** を試行（どちらかに問題 UUID が含まれればヒット）。
...
- **回帰テスト**: `npm run test:e2e`（Playwright、`/test?exam=ppl` の件数表示）
