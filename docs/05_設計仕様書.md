...
**最終更新**: 2025年12月24日（飛行計画エクスポート/インポート・燃料計算・印刷機能実装）
**バージョン**: Design Specification v3.3
...
**目的**: ユーザーのミッション達成を記録し、XPを付与する。

**パラメータ**:
- `p_user_id` (uuid): ユーザーID
- `p_mission_id` (uuid): 達成したミッションID
...
**ロジック概要**:
1. ミッション情報を取得（`xp_reward`を確認）
2. ユーザーの現在のXPとランクを取得
3. XPを加算し、ランクアップを判定
4. `user_missions`テーブルに達成記録を追加（`ON CONFLICT`で更新）
5. `profiles`テーブルの`xp_points`と`rank`を更新

**注意事項（2025年12月21日修正）**:
- `ON CONFLICT DO UPDATE`句で`xp_earned = EXCLUDED.xp_earned`を使用（曖昧性回避）
- デイリー/ウィークリーミッションの場合は、既に達成済みかチェック
...
#### **パフォーマンス最適化（2025年1月実施）**
- **auth.uid()の最適化**: すべてのRLSポリシーで`auth.uid()`を`(select auth.uid())`に変更
  - 各行での再評価を防ぎ、クエリパフォーマンスを向上
- **ポリシー統合**: 同じロール・アクションに対する複数のpermissiveポリシーを統合
  - `announcements`: SELECTポリシーを統合
  - `fan_photos`: SELECTポリシーを統合
  - `learning_content_comments`: 重複ポリシーを削除
...
-- 最適化: (select auth.uid())を使用
CREATE POLICY "enable_public_profiles_access" ON profiles
FOR ALL
USING (((select auth.uid()) = id) OR ((select auth.uid()) IS NULL))
...
-- 全ユーザーがアクティブなミッションを閲覧可能
CREATE POLICY "missions_select_active" ON missions
  FOR SELECT
  USING (is_active = true);

-- Adminは全ミッションを閲覧可能
CREATE POLICY "missions_admin_select_all" ON missions
  FOR SELECT
  USING (
    (select auth.uid()) IN (
      SELECT profiles.id
      FROM profiles
      WHERE profiles.roll = 'Admin'
    )
  );

-- Adminはミッションを管理可能（INSERT/UPDATE/DELETE）
CREATE POLICY "missions_admin_insert" ON missions FOR INSERT ...
CREATE POLICY "missions_admin_update" ON missions FOR UPDATE ...
CREATE POLICY "missions_admin_delete" ON missions FOR DELETE ...
```
...
-- ユーザーは自分のミッション達成記録を閲覧可能
CREATE POLICY "user_missions_select_own" ON user_missions
  FOR SELECT
  USING ((select auth.uid()) = user_id);

-- ユーザーは自分のミッション達成記録を挿入可能
CREATE POLICY "user_missions_insert_own" ON user_missions
  FOR INSERT
  WITH CHECK ((select auth.uid()) = user_id);

-- Adminは全ユーザーのミッション達成記録を管理可能
CREATE POLICY "user_missions_admin_select_all" ON user_missions FOR SELECT ...
CREATE POLICY "user_missions_admin_update" ON user_missions FOR UPDATE ...
CREATE POLICY "user_missions_admin_delete" ON user_missions FOR DELETE ...
```
...
#### **G. プロフィール設定画面（2025年12月21日更新）**

**レイアウト構成**:
- **タブナビゲーション**: プロフィール、セキュリティ、ソーシャル、通知の4タブ
- **プロフィールタブ**:
  - アバターアップローダー（左側）
  - 基本情報フォーム（右側）
    - ユーザー名（必須）
...
**UIスタイル仕様（2025年12月21日実装）**:
- **編集モード時の入力フィールド**:
  - 背景色: `bg-brand-secondary-light`（`#1a1a1a`）
  - テキスト色: `text-white`
  - ボーダー: `border-brand-primary/30`
  - フォーカス時: `focus:border-brand-primary focus:ring-2 focus:ring-brand-primary/20`
- **非編集モード時の入力フィールド**:
  - 背景色: `bg-brand-secondary/50`
  - テキスト色: `text-gray-100`
  - カーソル: `cursor-not-allowed`
- **適用範囲**: プロフィールタブ、セキュリティタブ、ソーシャルタブ、通知タブのすべての入力フィールド

**セキュリティタブ**:
- パスワード変更フォーム:
  - 現在のパスワード（表示/非表示トグル付き）
  - 新しいパスワード
  - パスワード確認
- パスワード情報表示:
  - 「最後に更新」日時の表示（日本語形式: "2025年12月21日"）
  - `password_updated_at`が`null`の場合は「不明」と表示

**入力フィールドアイコンのスタイル**:
- **時間入力フィールド**（`type="time"`）:
  - CSS疑似要素でカレンダー/時計アイコンを明るく表示
  - `filter: invert(1) brightness(2)`を適用
- **パスワード表示/非表示アイコン**:
  - アイコン色: `text-gray-300`
  - ホバー時: `text-white`
...
**ユーザーメニュー**:
- **メニュー項目**（2025年12月21日更新）:
  - プロフィール設定（`/account?tab=profile`へのリンク）
  - ログアウト
- **削除された項目**: アカウント設定（「プロフィール設定」と重複していたため）

**データベーススキーマ変更**:
- `profiles`テーブルに`password_updated_at`カラムを追加
  - 型: `timestamp with time zone`
  - デフォルト: `NULL`
  - インデックス: `idx_profiles_password_updated_at`
...
export interface FlightPlan {
  ...
  // ⭐追加 (2025年12月24日)
  aircraftId?: string;        // 機体プリセットID（例: 't4'）
  cruiseFuelFlowLbPerHr?: number; // 巡航燃料流量（lb/hr）
  taxiFuelLb?: number;        // タキシー燃料（lb）
  reserveFuelLb?: number;     // リザーブ燃料（lb）
  initialFuelLb?: number;     // 初期燃料（lb）
}
```

**変更内容** (2025年12月24日):
- 機体プリセット関連フィールドを追加（`aircraftId`, `cruiseFuelFlowLbPerHr`, `taxiFuelLb`, `reserveFuelLb`, `initialFuelLb`）
- 燃料計算に必要な情報を`FlightPlan`に統合

#### **RouteSegment型（拡張）** ⭐更新 (2025年12月24日)
```typescript
export interface RouteSegment {
  ...
  // ⭐追加 (2025年12月24日)
  fuelUsedLb?: number;        // このレグで使用した燃料（lb）
  fuelRemainingLb?: number;   // このレグ終了時の残燃料（lb）
  frequency?: string;         // 管制周波数（ACC/APP等）
}
```

**変更内容** (2025年12月24日):
- 燃料計算結果（`fuelUsedLb`, `fuelRemainingLb`）を追加
- 空域判定で取得した周波数（`frequency`）を追加
...
### **データ永続化とエクスポート/インポート機能** ⭐NEW (2025年12月24日実装)
...
export interface PlanDocumentV1 {
  ...
  planInput: FlightPlan;     // ユーザー入力データ
  derived?: {
    notes?: string;
...
**設計思想**:
- **バージョン管理**: `schemaVersion`で将来のスキーマ変更に対応
- **データ分離**: `planInput`（ユーザー入力）と`derived`（計算結果）を分離
- **単位系の明示**: データの解釈を明確化
- **拡張性**: `derived.extensions`で将来の機能追加に対応

#### **エクスポート/インポート機能**
- **エクスポート**: `toPlanDocument()`で`FlightPlan`を`PlanDocumentV1`に変換し、JSONファイルとしてダウンロード
- **インポート**: `fromPlanDocument()`でJSONファイルを読み込み、`FlightPlan`に復元（安全なデシリアライズ）
- **後方互換性**: スキーマバージョン不一致時は`null`を返し、エラーハンドリング

**実装ファイル**:
- `src/utils/planDocument.ts`: シリアライズ/デシリアライズロジック

### **機体プリセットシステム** ⭐NEW (2025年12月24日実装)

#### **AircraftPreset型**
```typescript
export interface AircraftPreset {
  id: string;                    // 機体ID（例: 't4'）
  name: string;                   // 機体名（例: 'Kawasaki T-4 (mock)'）
  cruiseFuelFlowLbPerHr: number; // 巡航燃料流量（lb/hr）
  taxiFuelLb: number;            // タキシー燃料（lb）
  reserveFuelLb: number;         // リザーブ燃料（lb）
  defaultInitialFuelLb: number;  // デフォルト初期燃料（lb）
}
```

#### **機体プリセットデータ**
- **データソース**: `src/data/aircraftPresets.ts`
...
#### **燃料計算ロジック**
- **レグごとの燃料消費**: `fuelUsedLb = legDurationHours × cruiseFuelFlowLbPerHr`
- **累積残燃料**: `fuelRemainingLb = initialFuelLb - taxiFuelLb - cumulativeFuelUsedLb`
- **表示形式**: 残燃料2500lb → 2.5（k-lb表示、1000で割った値）
- **単位系**: 燃料はlb、燃料流量はlb/hr、残燃料はk-lb

**実装ファイル**:
- `src/data/aircraftPresets.ts`: 機体プリセットデータ定義
- `src/components/flight/PlanningTab.tsx`: 機体選択UIと燃料計算ロジック

### **空域→周波数の自動紐づけ** ⭐NEW (2025年12月24日実装)

#### **空域判定ロジック**
- **データソース**: 既存の空域GeoJSON（`ACC_Sector_High.geojson`, `ACC_Sector_Low.geojson`, `RAPCON.geojson`）
- **判定方法**: レグの中点座標＋高度帯で該当ポリゴンを判定（Point-in-Polygon）
- **周波数取得**: GeoJSONの`properties.frequency`または`properties.freq`から取得
- **優先順位**: 複数空域に該当する場合の優先順位ルール（実装済み）

#### **AirspaceHit型**
```typescript
export interface AirspaceHit {
  frequency?: string;   // 管制周波数
  sourceId?: string;     // データソースID（例: 'ACC_Sector_High'）
  name?: string;         // 空域名
}
```

**実装ファイル**:
- `src/utils/airspace.ts`: 空域判定と周波数取得ロジック

### **印刷機能** ⭐NEW (2025年12月24日実装)

#### **PlanPrintViewコンポーネント**
- **レイアウト**: A4縦、2ページ構成
  - 1ページ目: NavLog（ログ表）
  - 2ページ目: Route Sketch（SVG経路図）
- **NavLog内容**: レグごとの飛行諸元（速度、磁方位、高度、距離、ETA、所要時間、使用燃料、残燃料、周波数）
- **SVG経路図**: 出発地（緑）→ウェイポイント（黒）→到着地（赤）を線で接続、各ポイントに番号表示

#### **印刷方式**
- **現状**: ブラウザの`window.print()`を使用（CSS `@media print`で制御）
- **課題**: 環境依存が大きく、空白ページが発生しやすい
- **今後の計画**: PDF生成ライブラリ（`@react-pdf/renderer`等）の導入を検討

**実装ファイル**:
- `src/components/flight/PlanPrintView.tsx`: 印刷専用ビュー
- `src/index.css`: 印刷用CSS（`@media print`）

### **将来の拡張予定**

#### **短期（1-3ヶ月）**
- **PDF生成ライブラリ導入**: ブラウザ印刷の代替として確実に2ページPDFを生成
...
## 🎨 テーマシステム設計（v1.2追加 → v2.0更新）

### **テーマ管理システム（v2.0: 固定スタイル化）**

**変更履歴**: 2025年12月21日 - `ThemeContext`を削除し、HUD固定スタイル（Dayモード）に統一。

#### **旧ThemeContext設計（削除済み）**
```typescript
// 削除: ThemeContext.tsx
...
- **スタイル制御**: Tailwind CSSクラスとCSS変数で直接制御
  - `bg-whiskyPapa-black`, `text-whiskyPapa-yellow`（Marketingページ）
  - `hud-surface`, `hud-text`, `hud-border`（Appページ）
  - `border-green-500/50`, `text-green-400`（HUDグリーン固定）

#### **移行理由**
- テーマ切り替え機能の複雑さを削減
- コードベースの簡素化と保守性向上
- HUDグリーン（Dayモード）を標準デザインとして統一
...
## 🎯 Whisky Papa ブランド機能設計（2025年1月実装、2025年12月21日統一完了）

### **Phase 1: Brand Foundation**

#### **レイアウト設計（2025年12月21日更新）**
- **MarketingLayout**: 全ページ統一レイアウト（黒×黄色基調）
  - ヘッダー: 固定ヘッダー（sticky）、ナビゲーションメニュー、ハンバーガーメニュー（モバイル対応）
  - フッター: 3カラムグリッド（Whisky Papa情報、Quick Links、Tools）
  - Framer Motionによるアニメーション
  - **統一対象ページ**: Home, About, Articles, Learning, Planning, Test, Mission
- **HUDテーマ削除**: 2025年12月21日にHUDテーマを完全削除し、Whisky Papaテーマに統一
  - 削除ファイル: `ThemeContext.tsx`, `HudPrimitives.tsx`, `ThemeToggler.tsx`, `AppLayout.tsx`
  - 全コンポーネントで`useTheme`依存を削除し、固定Whisky Papaクラスに置換
...
-- RLS（Row Level Security）の設定（2025年1月追加）
ALTER TABLE missions ENABLE ROW LEVEL SECURITY;
ALTER TABLE user_missions ENABLE ROW LEVEL SECURITY;

-- missionsテーブルのRLSポリシー
-- 全ユーザーがアクティブなミッションを閲覧可能
CREATE POLICY "missions_select_active" ON missions
  FOR SELECT USING (is_active = true);

-- Adminは全ミッションを管理可能
CREATE POLICY "missions_admin_select_all" ON missions FOR SELECT ...
CREATE POLICY "missions_admin_insert" ON missions FOR INSERT ...
CREATE POLICY "missions_admin_update" ON missions FOR UPDATE ...
CREATE POLICY "missions_admin_delete" ON missions FOR DELETE ...

-- user_missionsテーブルのRLSポリシー
-- ユーザーは自分のミッション達成記録を閲覧・挿入可能
CREATE POLICY "user_missions_select_own" ON user_missions
...
CREATE TABLE gallery_events (
  ...
  ends_at TIMESTAMPTZ, -- 投票締切日時（これを過ぎるといいね不可）
  created_at TIMESTAMPTZ DEFAULT now()
);
...
CREATE TABLE fan_photos (
  ...
  event_id UUID REFERENCES gallery_events(id) ON DELETE SET NULL, -- イベント紐づけ
  created_at TIMESTAMPTZ DEFAULT now(),
  updated_at TIMESTAMPTZ DEFAULT now()
);
...
-- いいね数集計ビュー
CREATE VIEW fan_photo_like_counts AS
SELECT
  photo_id,
...
-- インデックス
CREATE INDEX idx_fan_photos_event_id ON fan_photos(event_id);
CREATE INDEX idx_fan_photo_likes_photo_id ON fan_photo_likes(photo_id);
CREATE INDEX idx_fan_photo_likes_user_id ON fan_photo_likes(user_id);
CREATE INDEX idx_gallery_events_status ON gallery_events(status);
...
- **AdminEventPanel**: イベント管理パネル（管理者専用）
  - イベント作成・アクティブ化
  - タイトル、スラッグ、説明、開始日時、投票締切日時の設定
  - 新規イベント作成時に前イベントを自動アーカイブ
...
**主要機能**
- **イベント管理**: アクティブイベントとアーカイブイベントの管理
  - アクティブイベントは1つのみ（新規作成時に前イベントを自動アーカイブ）
  - イベントごとの写真フィルタリング
  - 投票締切日時（`ends_at`）を過ぎるといいね不可
  - 1ユーザー1イベント5枚の投稿上限（Adminは無制限）
- **いいね機能**: 承認済み写真へのいいね
  - アクティブイベントの写真のみいいね可能
  - 自分の写真にはいいね不可（DB側で強制）
  - 投票締切を過ぎたイベントの写真にはいいね不可（DB側で強制）
  - いいね数の集計と表示
- **Top3表示**: アクティブイベントの写真からいいね数上位3件を表示（1st/2nd/3rdバッジ付き）
- **ソート機能**: 新着順・いいね順の切り替え
- **タブ機能**:
  - **公開**: 全ユーザー向けギャラリー表示（アクティブイベント + 過去イベント）
  - **自分の投稿**: ログインユーザー向け（承認待ち/承認済みの分類表示）
  - **管理**: 管理者向け（イベント管理・承認管理）
- **過去イベント一覧**: アーカイブされたイベントとその写真を時系列で表示（折りたたみ可）
- **URL共有**: `/gallery?event=<slug>`でイベント指定表示に対応
...
- **fan_photo_likes**: 全ユーザーがいいねの追加・削除可能（承認済み写真のみ）
- **gallery_events**: 全ユーザーが閲覧可能、管理者のみ作成・更新可能
...
### **Gallery機能の段階的実装（2025年12月）**

#### **Phase 1: 管理タブ拡張（2025年12月26日）**
- **AdminReviewPanelコンポーネント**: 管理者専用の承認管理パネルを実装
  - 承認待ち写真の一覧表示（投稿者情報・日時・キャプション付き）
  - 写真の承認（`is_approved=true`設定）
  - 特集設定の切り替え（`is_featured`のON/OFF）
  - 却下機能（Storage削除 + DB削除）
- **Galleryページタブ機能**: 公開ギャラリー・管理タブの切り替えを実装
  - 管理者のみ「管理」タブを表示
  - `useAuthStore.profile.roll`によるAdmin判定を実装

#### **Phase 2: アップロード機能復旧（2025年12月26日）**
- **Supabase Storage設定**: `fan-photos`バケットを作成
  - 公開読み取り、認証ユーザー書き込み
  - 5MB制限、画像MIMEタイプ制限（JPEG/PNG/WebP/GIF）
- **Storage RLSポリシー**: `storage.objects`に適切なポリシーを追加
  - Public閲覧、認証ユーザーのアップロード・更新・削除
  - Adminによる削除権限（`lower(roll)='admin'`で判定）
- **UploadModal修正**: スクロール不能バグを修正
  - オーバーレイとモーダル本体に`overflow-y-auto`を追加
  - キャプション入力と送信ボタンが常に表示・操作可能に
- **エラーメッセージ改善**: アップロード失敗時のユーザー向けメッセージを改善

#### **Phase 3: いいね機能・Masonryレイアウト・自分の投稿タブ（2025年12月26日）**
- **いいね機能実装**:
  - `fan_photo_like_counts`ビューと`fan_photo_likes`テーブルを使用
  - `useGallery`フックにいいね数取得・ユーザーいいね状態取得・トグル機能を追加
  - 承認済み写真のみいいね可能（未承認はボタン非表示/無効化）
- **PhotoGrid拡張**: いいねUIを追加
  - 各写真サムネに❤️/🤍アイコンと件数表示
  - クリックでトグル（ログイン必須、未ログイン時は`/auth`誘導）
  - Lightbox内にもいいね表示を追加
- **人気Top3セクション**: アクティブイベントの写真からいいね数上位3件を表示
  - `likes_count desc` → `created_at desc`の優先順位でソート
- **ソート機能**: 新着順・いいね順の切り替えを実装
  - `sort=newest|likes`のstate管理
- **自分の投稿タブ**: ログインユーザー向けのタブを追加
  - 承認待ち/承認済みの分類表示
  - `useGallery.userPhotos`を利用
- **Masonryレイアウト**: CSS columnsベースの実装（依存追加なし）
  - `PhotoGrid`に`layout="grid"|"masonry"`のpropsを追加
  - `columns-1 sm:columns-2 lg:columns-3 xl:columns-4` + `break-inside-avoid`を使用
  - Masonry時は可変高さに切替

#### **Phase 4: イベント制Gallery実装（2025年12月26日）**
- **gallery_eventsテーブル**: イベント管理テーブルを作成
  - `id`, `title`, `slug`（URL共有用、UNIQUE）, `description`, `status`（`active|archived`）
  - `starts_at`, `ends_at`（投票締切日時）, `created_at`
- **fan_photosテーブル拡張**: `event_id`フィールドを追加
  - 投稿はアクティブイベントへ自動紐付け
  - 既存データは「Legacy」イベントへ移行
- **アクティブイベント制約**: 常に1つのみアクティブ
  - `ensure_single_active_event()`トリガー関数を実装
  - 新規イベントを`active`にすると、他の`active`を自動で`archived`に更新
- **投稿上限機能**: 1ユーザー1イベント5枚制限（Adminは無制限）
  - `enforce_event_photo_limit()`トリガー関数を実装
  - INSERT前チェックで上限超過を防止
- **いいねルール強化**: RLSポリシーでDB側制約を追加
  - 自分の投稿へのいいね禁止
  - 締切後は投票（いいね）の追加/取消を禁止
  - 承認済み・アクティブイベント・締切前のみいいね可能
- **AdminEventPanel拡張**: イベント作成・アクティブ化UIを追加
  - タイトル、スラッグ、説明、開始日時、投票締切日時の設定
  - 「このイベントをアクティブ化」ボタンで前イベント自動アーカイブ
- **Galleryページ構成変更**: イベント単位の表示に刷新
  - ページ上部: アクティブイベント（投稿・Top3・ギャラリー・投票）
  - ページ下部: 過去イベントを時系列で表示（折りたたみ可）
  - `/gallery?event=<slug>`でイベント指定表示に対応
- **UploadModal拡張**: イベント連動とガイド表示を追加
  - 対応形式（JPEG/PNG/WebP/GIF）、5MB制限、現在イベント名を表示
  - 投票/投稿締切、残り投稿枠（0〜5）を表示
  - 締切後は投稿不可表示
- **いいねUIのイベント対応**: 締切後はいいねUIを無効化
  - Top3バッジ（1st/2nd/3rd）とイベントURL共有機能を追加

### **飛行計画エクスポート/インポート・燃料計算・印刷機能（2025年12月24日）**

#### **JSON正本によるデータ永続化**
- **PlanDocumentV1スキーマ**: バージョン管理付きJSON形式で飛行計画を保存・共有可能に
- **エクスポート/インポート機能**: `src/utils/planDocument.ts`を新規作成
- **型定義拡張**: `FlightPlan`型に機体プリセット関連フィールドを追加

#### **機体プリセットシステム**
- **T-4練習機モック**: `src/data/aircraftPresets.ts`を新規作成
- **燃料計算ロジック**: レグごとの燃料消費量と残燃料を自動計算
- **単位系**: 燃料はlb、燃料流量はlb/hr、残燃料はk-lb表示

#### **空域→周波数の自動紐づけ**
- **空域判定ロジック**: `src/utils/airspace.ts`を新規作成
- **周波数取得**: GeoJSONの`properties`から周波数を自動取得
- **RouteSegment型拡張**: `frequency`フィールドを追加

#### **A4印刷機能**
- **PlanPrintViewコンポーネント**: `src/components/flight/PlanPrintView.tsx`を新規作成
- **2ページ構成**: NavLog（1ページ目）とRoute Sketch（2ページ目）
- **印刷用CSS**: `src/index.css`に`@media print`ルールを追加
- **今後の計画**: PDF生成ライブラリ（`@react-pdf/renderer`等）の導入を検討

### **UI/UX統一（2025年12月21日）**

#### **テーマ統一**
- **HUDテーマ削除**: `ThemeContext.tsx`、`HudPrimitives.tsx`、`ThemeToggler.tsx`、`AppLayout.tsx`を削除
- **Whisky Papaテーマ統一**: 全ページを`MarketingLayout`で統一
  - 統一対象: Home, About, Articles, Learning, Planning, Test, Mission
  - 全コンポーネントで`useTheme`依存を削除し、固定Whisky Papaクラスに置換

#### **ナビゲーション改善**
- **ハンバーガーメニュー実装**: モバイル対応のハンバーガーメニューを追加
  - 未ログイン時とログイン時でメニュー内容を切り替え
- **Missionページ統合**: FlightPlanner、Articles、CPL Quiz、Academyへのアクセスを「Tools」タブに統合
  - ログイン時のみ表示される統合アクセスポイント

### **データベース修正（2025年12月21日）**

#### **complete_mission関数修正**
- **問題**: `ON CONFLICT DO UPDATE`句で`xp_earned = xp_earned`が曖昧な参照エラー（42702）を発生
- **修正**: `xp_earned = EXCLUDED.xp_earned`に変更し、挿入値を明示的に参照
- **影響**: ミッション達成時のXP付与が正常に動作

#### **テスト結果保存修正**
- **問題**: `user_test_results`テーブルに`session_id`が設定されず、セッションと個別回答が紐付けられない
- **修正**: `TestPage.tsx`の`handleSubmitQuiz`関数で、`quiz_sessions`への挿入後に返された`id`を取得し、`user_test_results`の各レコードに`session_id`として設定
- **影響**: テスト結果の詳細分析と進捗追跡が可能に

### **フック修正（2025年12月21日）**

#### **useGamificationフック修正**
- **問題**: `user_missions`テーブルへのクエリで`.single()`を使用し、結果がない場合にエラーが発生
- **修正**: `.limit(1)`に変更し、明示的なエラーハンドリングと空結果チェックを追加
- **影響**: 初回ミッション達成時やクエリ結果がない場合でも正常に動作

---

**最終更新**: 2025年12月26日
**バージョン**: Design Specification v3.4
