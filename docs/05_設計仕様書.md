...
### **主要機能**
- **データ永続化**: JSON形式で飛行計画を保存・共有（PlanDocumentV1スキーマ）。ブラウザ内下書きは `localStorage` キー `flight-academy-plan-draft-v1`（デバウンス保存・「下書き消去」で初期化）
- **レイアウト**: `WeatherCacheProvider` は `PlanningMapPage` で1つだけ（計画タブ／地図タブで気象キャッシュ共有）。`lg` 以上は計画＋地図の2カラム、未満はタブ切替
- **地図モジュール**: `MapTab`（シェル）＋ `MapTabContent`（Leafletレイヤー）＋ `FlightPlanRouteLayer`（ルート線・マーカー）＋ `map/hooks/*`（リージョン・NAVAID取得、カーソル、ダブルクリック追加）
