...
## 7. 試験明け・検討深化アイテム（Quiz Hub UX / 成長）

**決定（2026-07-14）**: 学科試験前の正念場では **UI/UX 大改修を凍結**。Action1（Planning チャンク安定化・SW `v4`）のみ本番投入済。以下は **試験明けに検討を深め、承認後に実装**する。

**根拠データ（GA4・2026-07 直近）**:

| 観察 | 数値の目安 | 示唆 |
|------|------------|------|
| 科目タブ完走率 | start→complete が診断より低い（例: subject ~15% vs diagnostic ~60%、ディメンション付き・母数小） | 摩擦は「科目選択〜開始〜完了」に偏る可能性 |
| 流入 | Direct + Google OAuth 戻りが主。Organic Search は極小 | 新規獲得チャネルが弱い |
| `/test` 依存 | PV の過半がクイズ | 完走 UX の改善は ALPM に直結しやすい |

### 7.1 Action 2 — 科目タブ完走率（UX）

| ID | 検討項目 | 種別 | メモ |
|----|----------|------|------|
| A2-a | 科目タブの **デフォルト問数を下げる**（例: 10→5）または「短いセット」を先頭 | 挙動・要承認 | 完走率改善の最小実験 |
| A2-b | 開始前に **所要時間・問数の明示** + 進捗バー | 文言/UI・要承認 | 離脱の心理的ハードル |
| A2-c | 科目フィルタの **段階開示**（主科目 → 問数 → 開始） | UI・要承認 | モバイル誤タップ・迷い |
| A2-d | GA4 `tab`×`count`×完了率の **週次監視**（実装済みパラメータ活用） | 計測のみ | 試験週〜明け直後も継続可 |

**着手条件**: 試験明け + GA4 で subject 完走が継続的に低いことの再確認 + UI 変更は明示承認。

### 7.2 Action 3 — トラフィック回復（成長）

| ID | 検討項目 | 種別 | メモ |
|----|----------|------|------|
| A3-a | 気象・通信記事の **検索意図タイトル／メタ**整備（PMF スライス） | コンテンツ | Organic の種火 |
| A3-b | 記事末尾の **記事→`/test` CTA** 露出強化（RelatedTests 既存） | 文言/導線・要承認 | ALPM（学習→演習） |
| A3-c | cohort **週次リマインド**文面・頻度の見直し | 運用 | Direct リテンション |
| A3-d | コミュニティ向け **週1学習スニペット**（手運用） | 運用 | 新規獲得の補助 |
| A3-e | GA4 **landing × newUsers** 週次確認 | 計測 | 入口の特定 |

**着手条件**: 試験明け。エンジニアリングよりコンテンツ／運用を先に検討してよい。

### 7.3 試験週中にやってよいこと（凍結対象外）

- Sentry `#4` / `chunk_recovery_reload` の監視
- A2-d / A3-e の計測確認（コード・UI 変更なし）
- コンテンツ執筆（PPL Subject 4 等）— Hub UX と独立
...
| 日付 | 内容 |
|------|------|
| 2026-07-14 | §7 追加 — 試験明け Action2/3 検討深化アイテム。試験週は UI 凍結。 |
