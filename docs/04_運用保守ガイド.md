...
**最終更新**: 2025年9月23日
**バージョン**: Operations & Maintenance Guide v1.3
...
- 対象: 記事ID `1.3.6_BlessingAndAdmonition`（2025年11月2日追加）ほか

### 前提チェック
1. MDX ファイル名（拡張子なし）が `learning_contents.id` と一致していること
2. カテゴリが Articles 対象（『メンタリティー』『思考法』『操縦』）に合致
3. `is_published = true`
4. シリーズ記事の場合、`series`と`order`がメタデータに設定されていること（順次アンロック機能用）

### リリース手順（SupabaseMCP）
1. プロジェクト確認: `FlightAcademy (project_id: fstynltdfdetpyvbrswr)`
2. 以下のUpsert SQLを実行（例: 1.3.6_BlessingAndAdmonition）
...
INSERT INTO learning_contents (
  ...
  'メンタリティー',
  'ジェット機課程修了を祝福し、「慣れ」という悪魔と戦い続ける覚悟を説く。初心を忘れず、背負う翼の重みを知れ。',
  136,
  NULL,
  'article',
  TRUE,
...
WHERE id = '1.3.6_BlessingAndAdmonition';
```
...
- **シリーズ順次アンロック機能**: シリーズ記事は`meta.series`と`meta.order`を必ず設定すること。未ログインユーザーは1話のみ閲覧可能、ログインユーザーは前話読了で次話が解放される。
...
**最終更新**: 2025年1月15日
**バージョン**: Operations & Maintenance Guide v1.4
**管理者**: FlightAcademy開発チーム

---

### Articlesページテーマ統一運用ガイド（2025年9月23日 追加）

#### **テーマ統一の運用ルール**
- **カードデザイン統一**
  - 角丸: すべて`rounded-xl`を使用
  - 枠線: すべて`border-2`（2px）を使用
  - シャドウ: すべて`shadow-lg`を使用
  - パディング: `p-5`（メインカード）または`p-3 sm:p-4`（統計カード）を使用

#### **テーマ対応カラーの運用**
- **Dayテーマ**
  - 枠線: `border-green-500/50`
  - シャドウ: `shadow-green-900/10`
  - テキスト: `text-[#39FF14]`
  - 背景: `#0b1d3a`（NavyBlue）
- **Darkテーマ**
  - 枠線: `border-red-500/60`
  - シャドウ: `shadow-red-900/20`
  - テキスト: `text-white`
  - 背景: `var(--bg)`（CSS変数）
...
- **ArticleSearch & ArticleSortControls**
  ...
  - 枠線: `border-2` + テーマ対応色
- **ProgressSidebar**
  - 全カード: `rounded-xl border-2 shadow-lg p-5`
  - テーマ対応の枠線色を適用
- **EnhancedArticleCard**
  - 角丸: `rounded-xl`
  - 枠線: `border-2` + テーマ対応色
...
## 🔐 記事カード統計データ表示統一運用ガイド（2025年9月23日 追加）

### **統計データ表示の統一運用**

#### **ログイン状態に応じた表示制御の運用**
- **ログインユーザー**: いいね数、コメント数、閲覧数がすべて表示される
- **匿名ユーザー**: 閲覧数のみ表示、いいね・コメント数は非表示
- **ログイン促進**: 匿名ユーザーがいいね・コメント機能を使用しようとした場合、ログイン促進メッセージを表示

#### **統計データ読み込みの運用**
- **読み込みタイミング**: 記事コンテンツがロードされた後に統計データを自動読み込み
- **デフォルト値**: 統計データが存在しない記事でも統一された表示を確保
- **エラーハンドリング**: 統計データ読み込み失敗時はデフォルト値（0）を表示

#### **閲覧数自動カウントの運用**
- **カウントタイミング**: 記事タイトルをクリックした時点で閲覧数を記録
- **記録対象**: ログイン状態に関係なく、すべての記事アクセスをカウント
- **エラー処理**: 閲覧数記録失敗時はエラーログを出力するが、ユーザー体験には影響しない

### **ログイン制限機能の運用**

#### **いいね機能の制限**
- **ログインユーザー**: 正常にいいね機能が利用可能
- **匿名ユーザー**: いいねボタンクリック時に「いいねするにはログインが必要です」メッセージを表示
- **セッション管理**: 匿名ユーザー向けのセッション管理機能は削除済み

#### **コメント機能の制限**
- **ログインユーザー**: 正常にコメント機能が利用可能
- **匿名ユーザー**: コメント投稿時に「コメントするにはログインが必要です」メッセージを表示
- **データベース制限**: SupabaseのRLS（Row Level Security）でバックエンド側でも制限

### **統一された表示の運用**

#### **公開日の表示**
- **優先順位**: `articleMeta.publishedAt` → `article.created_at` の順で表示
- **フォーマット**: 日本語形式（例：2025年9月23日）
- **表示条件**: どちらかの日付が存在する場合のみ表示

#### **統計データの表示**
- **いいね数**: ログインユーザーのみ表示、ハートアイコン（❤️/🤍）で状態表示
- **コメント数**: ログインユーザーのみ表示、💬アイコンで表示
- **閲覧数**: 常に表示、👀アイコンで表示
...
2. **ネットワークタブ**でSupabaseへのAPI呼び出しが成功しているか確認
3. **認証状態**を確認（ログインしているかどうか）
4. **記事コンテンツ**が正常に読み込まれているか確認

#### **閲覧数がカウントされない場合**
1. **記事タイトルクリック**時にコンソールエラーがないか確認
2. **Supabaseの`learning_content_views`テーブル**にデータが記録されているか確認
3. **ネットワーク接続**が正常か確認

#### **ログイン制限が機能しない場合**
1. **認証状態**が正しく取得できているか確認
2. **`useAuth`フック**が正常に動作しているか確認
3. **Supabaseの認証設定**が正しく構成されているか確認

### **パフォーマンス最適化**

#### **統計データ読み込みの最適化**
- **バッチ読み込み**: 複数の記事の統計データを一度に読み込み
- **キャッシュ活用**: 既に読み込んだ統計データは再利用
- **遅延読み込み**: 記事コンテンツが読み込まれた後に統計データを読み込み

#### **表示パフォーマンスの最適化**
- **条件付きレンダリング**: ログイン状態に応じて必要な要素のみレンダリング
- **メモ化**: `useMemo`を使用して統計データの計算を最適化
- **エラーバウンダリ**: 統計データ読み込み失敗時の適切な処理

### **監視とメンテナンス**

#### **定期チェック項目**
- **統計データの整合性**: いいね数、コメント数、閲覧数の整合性確認
- **ログイン制限の動作**: 匿名ユーザーでの制限が正しく動作しているか確認
- **閲覧数カウント**: 記事アクセス時の閲覧数記録が正常に動作しているか確認

#### **ログ監視**
- **エラーログ**: 統計データ読み込み失敗、閲覧数記録失敗のログを監視
- **パフォーマンスログ**: 統計データ読み込み時間の監視
- **ユーザー行動ログ**: ログイン状態に応じた機能利用状況の監視

---

## 📝 記事作成ガイドライン：著作権対策（2025年1月15日 追加）

### **書籍引用の適正化**

#### **適正な引用の要件**
1. **主従関係の明確化**: 自身の文章が「主」、引用が「従」であること
2. **引用部分の明確な区別**: カギ括弧（『』）や`<Highlight>`タグで明示
3. **引用の必要性**: 意見の補強・具体例として必要最小限の範囲
4. **出典の明示**: 書籍名、著者名を必ず記載
5. **改変禁止**: 原文のまま引用（要約は自身の文章として記載）

#### **書籍引用の形式例**
```mdx
// 本文中での引用
<Highlight>**「失敗は成功の一部である」**</Highlight>という考え方（出典：『ユダヤ人大富豪の教え』）

// 記事末尾の参考書籍セクション
## 参考書籍

- [『7つの習慣』（スティーブン・R・コヴィー）](https://www.amazon.co.jp/dp/4906638015/?tag=your-tag-here)
- [『GIVE & TAKE 「与える人」こそ成功する時代』（アダム・グラント）](https://www.amazon.co.jp/dp/4478020287/?tag=your-tag-here)
```

### **映画・作品コンテンツの扱い**

#### **直接的な引用の回避**
- **映画タイトル**: 「トップガン マーヴェリック」→「あの伝説的な航空アクション映画」「戦闘機乗りのエリートたちを描いた名作」
- **キャラクター名**: 直接的な名称を避け、ぼかした表現を使用
  - 「トム兄貴」「マーヴェリック」→「トム兄ぃ」
  - 「アイスマン」→「アイスの旦那」
  - 「グース」→「グースの兄貴」
  - 「ルースター」「ハングマン」→「ルースターの若造」「ハングマンの若造」

#### **英語セリフの扱い**
- **直接引用の削除**: 英語のセリフを直接引用しない
- **日本語要約**: キャラクター設定に沿った日本語で要約
  - 例: "You can be my wingman anytime." → 「お前はいつでも俺の相棒ばい！」（道真公の博多弁）
  - 例: "It's not the plane, it's the pilot!" → 「重要なのは機体じゃなか、パイロットばい！」

#### **オマージュ・パロディとしての表現**
- **キャラクター設定の活用**: 道真公が「あの映画の大ファン」という設定を維持
- **概念の借用**: 直接的な名称ではなく、概念や設定を拝借
- **雰囲気の模倣**: セリフや名称そのものではなく、「らしさ」を感じさせるシチュエーションを描写

### **記事フォーマット仕様**

#### **参考書籍セクションの追加**
すべての書籍引用を含む記事の末尾に、以下の形式で「参考書籍」セクションを追加する：
...
#### **実装済み記事**
以下の記事に参考書籍セクションが追加済み：
- `1.1.1_UnconsciousSuccess.mdx` - 7つの習慣
- `1.1.2_EndWithFuture.mdx` - 7つの習慣
- `1.1.3_PrioritizingMostImportant.mdx` - 7つの習慣
- `1.1.6_Synergize.mdx` - 7つの習慣
- `2.4.1_MillionaireTeaching1.mdx` - ユダヤ人大富豪の教え
- `1.2.6_GiveAndTake.mdx` - GIVE & TAKE
...
新規記事作成時は以下の項目を確認：
- [ ] 書籍引用は適正な範囲内で、出典を明記
- [ ] 映画タイトル・キャラクター名を直接使用していない
- [ ] 英語セリフは日本語要約に変更済み
- [ ] 記事末尾に「参考書籍」セクションを追加（書籍引用がある場合）
- [ ] Amazonアソシエイトリンクが正しく設定されている
- [ ] キャラクター設定（道真公の博多弁など）が一貫している
