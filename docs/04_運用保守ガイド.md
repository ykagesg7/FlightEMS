...
- **動作確認**: 本番で **`/debug/sentry-test`**（一時ページ）のボタンから `captureException` するか、ブラウザコンソールで例外を投げる、または Sentry の「Send test event」を使う。ダッシュボードの **Issues** にイベントが出れば成功。**検証後は** `App.tsx` の当該ルートと `src/pages/debug/SentryTestPage.tsx` を削除すること。
