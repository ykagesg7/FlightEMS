...
- **全体（MDX 本数 / シラバス 150 論点）**: **39/150（26.0%）** — `src/content/lessons/PPL-*.mdx` 実数（2026-06-28）
- **Phase 1 相当の記事**: 工学 20 本 + 法規 14 本 + 気象 Phase 1 **5 本**のうち Phase 1 優先トピックを **おおむねカバー**（厳密な論点対応は各 Subject 節の ✅ を正）
- **Phase 2 / 3**: 工学 Phase 2 記事 2 本ほか。Phase 3 は未着手

**KPI 正本**: Phase C 目標 **25/150 以上**（[01](01_Current_Status_and_Roadmap.md) C-7）— **達成済**。Subject 2 Phase 1 は **5/12 本**（`learning_contents` **201〜205** 登録済）。
...
| **Subject 2: 航空気象** | 35 | **5** | 14.3% | 5/12 | 0/15 | 0/8 |
...
> **執筆優先順・中・小分類マッピング**: [10_航空工学_学科試験攻略ブログ_ロードマップ.md](10_航空工学_学科試験攻略ブログ_ロードマップ.md) を参照（4ChoiceQuiz AD問題データに基づく推奨順序）。
...
- [x] **Phase 1** 操縦性: 舵の効き、トリム、三舵（エルロン・エレベータ・ラダー）の働き
  - ✅ [PPL-1-1-11_ControlSurfaces](../src/content/lessons/PPL-1-1-11_ControlSurfaces.mdx) - 三舵と操縦性、トリム、逆エルロン効果の入口
...
- [x] **Phase 1** 燃料供給系統: システム概要、ベーパー・ロック現象
  - ✅ [PPL-1-2-8_FuelSystemVaporLock](../src/content/lessons/PPL-1-2-8_FuelSystemVaporLock.mdx) - ベーパー・ロックの原理と対策の方向性
- [ ] **Phase 2** 空調系統: 暖房・換気システムの概要
- [ ] **Phase 1** 着氷: 発生メカニズムと防止装置
- [ ] **Phase 1** 電気系統: 回路保護（ブレーカー・ヒューズ）、発電機とバッテリー

#### 1-2-2. 表示系統（計器）
- [x] **Phase 1** 航空計器一般: Tスキャン、注意警報の方法
  - ✅ [PPL-1-2-9_TScanInstrumentScan](../src/content/lessons/PPL-1-2-9_TScanInstrumentScan.mdx) - T スキャンと計器監視の型
...
- [x] **Phase 1** 安定度: 断熱減率、大気の状態曲線、安定・不安定の判定
  - ✅ [PPL-2-1-5_AtmosphericStabilityBasics](../src/content/lessons/PPL-2-1-5_AtmosphericStabilityBasics.mdx) — DALR/SALR/ELR・不等式判定・CB迂回（2026-06-28）
...
- `docs/10_航空工学_学科試験攻略ブログ_ロードマップ.md` - 航空工学の詳細執筆計画
