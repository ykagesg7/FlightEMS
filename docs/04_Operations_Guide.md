...
- **方針**: 週末に MDX をまとめてデプロイ。公開は **JST の `publishedAt`（MDX）+ `learning_contents.is_published`（DB）** で日次制御。毎日の git コミットは不要。既定ペースは **週 3 本（日・水・金）** — **1 本目は日曜 17:00 案内メールと同時に読めるよう `publishDate` を日曜**、2・3 本目は水・金。ドリップは CP が先（W34: 1-1〜1-3／8/17・19・21。W35: 2-1〜2-3／8/24・26・28。W36: 2-4〜2-6／**8/30**・9/2・9/4。W37: 3-1・3-2・4-1／**9/6**・9/9・9/11）。FN 1-4 以降はストックしてから schedule に載せる（[FN 正本](content_outlines/FN_Formation_2026/README.md)）。
...
- **手動実行**（本番）:
  ...
  curl -sS -H "Authorization: Bearer $CRON_SECRET" "https://flight-lms.vercel.app/api/cron/article-weekly-digest?isoWeek=2026-W33"
  ```
...
タグ健常性の確認とは別に、**アクセスとエラーの判断ログ**は **火曜 09:00 JST**（ISO 週: 月〜日）に [ops/Weekly_Telemetry_Review.md](ops/Weekly_Telemetry_Review.md) へ追記する。数字は `weekly-telemetry-ga4` の artifact。Slack Facts は日本語（`weekly-telemetry-notify`、`@` なし）。正本 PR は `weekly-telemetry-draft-pr`（自動）。Skill `weekly-telemetry-review` は任意。L0 は Facts スレッドへ `APPROVE-DOC` / `HOLD`（Slash Command なし。`weekly-telemetry-approve`）。L1 は許可リスト空。別チャットの Agent は正本を先に読む。単発の Quiz Hub ファネルは [`artifacts/quiz_hub_funnel_memo_2026-07-08.md`](../artifacts/quiz_hub_funnel_memo_2026-07-08.md)。
