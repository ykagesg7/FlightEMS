...
1. **単一ページでのみ使用されるコンポーネント**
   ...
   - 例: `src/pages/mission/components/MissionRankSection.tsx`
...
#### MDX 記事本文（`MDXContent.tsx` / `CalloutComponents.tsx`）
...
- **`src/layouts/MarketingLayout.tsx`**: 共通ヘッダー・フッター。`NavLink` で現在ルート強調、主ナビは **ARTICLES**（`/articles`）／**QUIZ**（`/test`）等。ロゴは `F2favicon.png` をパネル色＋リングで囲み、タイポで「Flight Academy」「Learn Smart.」を表示。モバイル展開パネルは **`useMobileNavFocusTrap`**（Tab 循環・Escape・フォーカス復帰）。
- **`src/hooks/useMobileNavFocusTrap.ts`**: 上記モバイルメニュー専用（新規 npm 依存なし）。
...
│       ├── RelatedArticles.tsx
│       ├── RelatedTestsBlock.tsx   # learning_test_mapping → /test 導線
│       ├── ScrollToButtons.tsx
│       ├── TableOfContents.tsx
│       ├── AISearchEngine.tsx
│       ├── usePrevNext.ts
