...
| 2026-05-12 | **5-4-1〜5-4-7**（空域・機長権限・SPC概要・安全阻害・報告・地上運用・最低高度入門）を `learning_contents` に追加: [`20260512_learning_contents_ppl_subject5_508_514.sql`](../scripts/database/20260512_learning_contents_ppl_subject5_508_514.sql)（`order_index` 508〜514）。MDX はリンク stem 正規化・`meta.tags` のグルメ由来削除・開発用ブリーフ削除を反映 |
