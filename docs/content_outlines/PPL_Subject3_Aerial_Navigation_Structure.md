...
1. **`PPL-3-1-1`** — 全シリーズ入口。緯度・経度・時差は以降すべての前提。
2. **`PPL-3-1-2`** — 高度は PPL-1-2-2 復習リンク必須。
3. **`PPL-3-1-3`** → **`PPL-3-1-4`** → **`PPL-3-1-5`** — 図→風→計画の依存順。
4. **`PPL-3-2-1`** → **`PPL-3-2-2`** — 地文のあと無線。
5. **`PPL-3-3-1`** — 法規 `PPL-5-4-1` へ相互リンク。
6. **`PPL-3-3-2`** — Phase 1 締め。HP 大量設問の入口。

**週次目安（C-7 二次）**: Phase 1 **2 本/週** × 5 週（CPL マッピングとトレードオフ可）。
...
|----------|-------------|------|
| 3.4.1 | — | `PPL-3-1-1`〜`2`・`3-2-1` 執筆後 |
| 3.4.2 / 3.4.5 / 3.4.6 | — | `PPL-3-2-2` 執筆後 |
| 3.4.4 | 部分（PPL 飛行計画 Callout あり） | `PPL-3-1-5` で差し替え・追加 |
