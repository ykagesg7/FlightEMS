...
- **ジョーの笑い（口頭試問の型は話ごとに選ぶ）:** 下表から **1 型** を選ぶ。**連続する話で同じ型を繰り返さない**。瓶バン＋「ひぇぇっ」は **連続話で使わない**（間に別型を挟む）。手順の講義を口頭試問に詰め込まない。比喩は Holding と揃える。ブルシットの対象は人格ではなく、説明の長さ／誤った忙しさ／意地。締めは短い行動指示。

| ID | 型名 | 流れ（要約） | 向く話 | 例 |
|----|------|--------------|--------|-----|
| A | **遅延ブルシット** | 誤答 → 反論 → ブルシット遅延 → 短い指示。瓶バンなし | 単純な禁止行為・待ち | 3-1「遅れを待て」 |
| B | **瓶バン** | 誤答 → 反論 → 空き瓶バン（＋任意でひぇぇっ）→ 短い指示 | 意地・粘り・形完遂の誘惑 | 3-2「兆候を見たらリカバリー」 |
| C | **時間圧迫** | 短問い → 長い手順読み上げ →「もう○○だ」→ 反論 → 遅延ブルシット → 短い指示 | 進行中の緊急感 | 2-1「もう沈んでる」 |
| D | **比喩一刀** | 誤答 → Holding 比喩で一発 → 反論 → 短い指示。瓶バンなし | Holding が強い話 | 引き戸／鍋／自転車など |
| E | **分岐突きつけ** | ひどさ／分岐を突きつける → 学生が一律解 → 正しい分岐へ強制 → 短い指示 | 手順が分岐する話 | Nose-high／Nose-low |
| F | **復唱切り** | Brief 一句を求める → 学生がレシピ音読 → 途中で切る → 短い指示 | 「言える一句」が主題 | 原則の暗唱確認 |

執筆時は MDX 末尾コメントか README 変更履歴に `口頭試問: 型X` を残すと連続回避しやすい。
...
| 2-4 | `CP-2-4_Unload` | `cp-2-4-unload` | 4 | Acceleration / stab demo | **5.13, 5.18** | **精緻化済・W36 日 8/30** |
...
| 3-2 | `CP-3-2_PatternATS` | `cp-3-2-pattern-ats` | 2 | Pattern approach-to-stall | **5.22–5.25** | **29** | **精緻化済・W37 水 9/9** |
...
| 2026-09-06 | ジョーの笑いを **遅延ブルシット／瓶バンの選択制**へ（連続話で瓶バン禁止）。`CP-3-2` 精緻化（瓶バン型。3-1 は遅延型）。 |
| 2026-09-06 | 口頭試問を型 A〜F に拡充。`CP-4-1` 精緻化（型 E 分岐突きつけ）。 |
