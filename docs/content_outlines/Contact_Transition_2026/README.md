...
- **ジョーの笑い（口頭試問の型は話ごとに選ぶ）:** 次の **どちらか一方** を選ぶ。**連続する話で同じ型を繰り返さない**（特に瓶バン＋「ひぇぇっ」は **連続話で使わない**）。
  1. **遅延ブルシット型:** 誤答 → 学生の反論 → ブルシットを遅らせる → 短い行動指示（例: 3-1「遅れを待て」）。瓶バン／ひぇぇっは使わない。
  2. **瓶バン型:** 誤答 → 学生の反論 → 空き瓶バン＋「ひぇぇっ」→ 短い行動指示。手順の講義を口頭試問に詰め込まない。
  比喩はその話の Holding と揃える（例: 2-1 水炊き、3-1 競艇／ゴムボート、3-2 交差点の自転車）。ブルシットの対象は学生の人格ではなく、説明の長さ／誤った忙しさ／意地。締めは短い行動指示。
