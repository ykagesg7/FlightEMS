# FN 編隊シリーズ — 執筆正本（Season 1 + 2）

**名称**: FN（旧 FMT Season 1）。ファイル名・slug・`learning_contents.id` は `FMT-*` / `fmt-*` のまま（公開 URL と DB を変えない）。
...
| **ドリップは CP が先** | W34 は CP 1-1〜1-3。W35 は CP 2-1〜2-3。W36 は CP 2-4〜2-6。W37 は CP 3-1・3-2・4-1。FN 1-1〜1-3 は倉庫 |
| **FN はストック** | 1-4 以降は書いて倉庫へ。即公開しない |
| **1 週 1 シリーズ** | その週の digest は FN か「訓練の当たり前」か、どちらか |
| **週次メール** | ストックを `WEEKLY_ARTICLE_DIGESTS` に載せてから。日曜 17:00 JST より前にデプロイ |

**W34（2026-08-17 / 19 / 21）**: CP 1-1 → 1-2 → 1-3。FN はこの週に出さない。日曜（8/16）17:00 案内の対象。本番デプロイが前提。  
**W35（2026-08-24 / 26 / 28）**: CP 2-1 → 2-2 → 2-3。FN はこの週も出さない。日曜（8/23）17:00 案内の対象。  
**W36（2026-08-30 / 9/2 / 9/4）**: CP 2-4 → 2-5 → 2-6（Season 2 完結）。1 話目は日曜案内と同時。FN はこの週も出さない。

**W37（2026-09-06 / 9/9 / 9/11）**: CP 3-1 → 3-2 → 4-1（Season 3 完結＋Nose-high 前倒し）。FN はこの週も出さない。日曜（9/6）17:00 案内の対象。
...
Season 1 全 **10 話**＝週3本なら約 **3.5 週**。W34 は CP。FN のドリップ開始週は未定。
...
3. Cursor が翻訳済み教範 + 直近の公開 FN のトーンでレビュー。
4. 完成後のみ `src/content/lessons/` に MDX を置く。
5. `learning_contents` を **ストック登録**（`is_published: false`）。公開週が決まるまで schedule に載せない。

**Season 1 は 1-10 までストック済。** 読む順は **CP → FN**。W34 ドリップは CP 1-1〜1-3。[Contact_Transition_2026](../Contact_Transition_2026/README.md)。Season 2 のブリーフは求められたときだけ。全 18 本を一括起票しない。
...
- 詳細解説の `###` は **PREP**（`P（Point）` / `R（Reason）` / `E（Example）` / 締めの `P（Point）`）。Tips は PREP の外でよい
- 口頭試問の話者は **学生：** / **学生（白目）：**（`お前（学生）` は使わない。ジョー台詞内の「お前」は二人称として残す）
...
| 9 | `FMT-1-9_TacticalLAB` | `fmt-1-9-tactical-lab` | 9 | LAB / tactical turns / lookout | IFF §3.6、Fund 6.26–6.31、6.38 | **ストック** |
| 10 | `FMT-1-10_LostWingman` | `fmt-1-10-lost-wingman` | 10 | Lost wingman / KIO | Vol.3 §6.7、Fund 6E | **ストック** |
...
| 4-ship（名前だけ） | **1-9** 末尾の短い注記 |
| BD check / formation landing | **1-10** 末尾の短い注記 |
...
Season 1 の編隊話が数本できてからブリーフを書く。Heat-to-Guns 手順や CAS 9-Line は FN に載せない。
...
### Lost wingman / KIO（1-10 用）

| 項目 | 値 | 出典 |
|------|-----|------|
| 2-ship wings-level | lead に同時に知らせ、離れる方向へ **15° bank × 15 s** | Vol.3 §6.7.1 |
| 2-ship 旋回外側 | 旋回を reverse、**15° × 15 s** | Vol.3 §6.7.1 |
| 2-ship 旋回内側 | いったん power を落として nose-tail、lead に roll out を知らせる。bank は維持 | Vol.3 §6.7.1 |
| Number 4 | wings-level / 外側は **30° × 30 s**。内側は power 減 + **bank +15°**。#3 も離れたと仮定 | Vol.3 §6.7.2 |
| 最終進入 | 離れる・climb・inform・MAP / published missed。missed は assigned + **500 ft** | Vol.3 §6.7.1 |
| Both blind（編隊内 lost sight） | lead が高度差 **最低 1,000 ft** | Fund 6.64.6 |
| Tactical blind（双方） | deconflict **最低 500 ft**。**ET の slant 500 ft KIO（1-7）と混ぜるな** | Vol.3 §3.16.2.2 |
| KIO 後 | prebrief 安全高度（**最低 1,000 ft AGL**）へ | Fund 6.9 |
| 練習 | **day VMC のみ**。復帰は lead の **permission** | Vol.3 §6.7.5–6.7.6 |
| Vol.2 | Lost Wingman 専用 Area **なし**。主は **16 Safety**（critical）。lost sight で離れないのは **85 Breakout**。**81 は 1-9** | Vol.2 Table 3.1 |
...
- 公式訓練・検定の代替に聞こえる書き方（「これで FN 合格」等）
...
SQL 先例: `20260616`（1-1）〜`fmt_lost_wingman.sql`（1-10 ストック）。Season 2 は未起票。
...
| 2026-08-16 | W34 ドリップから FN 1-1〜1-3 を外し、`publishedAt` を削除（倉庫）。W34 は Contact。 |
