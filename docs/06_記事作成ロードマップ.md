...
**作成日**: 2025年1月16日
**最終更新**: 2026年4月12日（Phase 1 KPI 19 本の定義整理・[CPL_Phase1_KPI_Tracker.md](db/CPL_Phase1_KPI_Tracker.md) 新設）
**バージョン**: v1.3.0
...
**推奨読み順**: [docs/README.md](README.md) → [01_プロジェクト概要ガイド.md](01_プロジェクト概要ガイド.md) → このドキュメント

**関連（記事パイプライン）**: [07_PPL_Master_Syllabus.md](07_PPL_Master_Syllabus.md)（PPLシラバス）、[08_Syllabus_Management_Guide.md](08_Syllabus_Management_Guide.md)（命名・DB）、[10_航空工学_学科試験攻略ブログ_ロードマップ.md](10_航空工学_学科試験攻略ブログ_ロードマップ.md)（航空工学詳細）、[11_ドキュメント整合性検討.md](11_ドキュメント整合性検討.md)（4文書の役割）
...
### KPI「19本」の正本（[03](03_計画改善ロードマップ.md)・[00](00_Flight_Academy_Strategy.md) と同期）

[03](03_計画改善ロードマップ.md) の **5/19・10/19・19/19** は、次の **19 執筆単位**（`learning_contents.id`）の **本文化** 件数で数える。**MDX ファイル総数や下記「拡張・気象」枠は含めない**。
...
**進捗表（本文化・マッピング・更新日）の単一ソース**: [db/CPL_Phase1_KPI_Tracker.md](db/CPL_Phase1_KPI_Tracker.md)

### 記事リスト（拡張・並行バックログ）

以下は **KPI 19 本に加えて** [14](14_記事単元網羅とバックログ.md)・Phase 2 と並行して追う単元である（スタブ配置や本文化の進捗は 14 と MDX を正とする）。

#### 航空気象（ハブ＋`3.3.*` スタブ配置済み・本文化は継続）

> **整合（2026-04-12）**: `3.3.1`〜`3.3.12` の **スタブ MDX は `src/content/lessons` に配置済み**。`learning_test_mapping` は **12/12 本**（`3.3.8` / `3.3.12` は [20260412 SQL](../scripts/database/20260412_learning_test_mapping_meteo_338_3312.sql) で追補）。入口は引き続き `weather_basics` と `CPL-Hub-Meteorology`。数値の正本は [14_記事単元網羅とバックログ.md](14_記事単元網羅とバックログ.md)。
...
> **KPI 19 本に含まれるのは `3.2.1`〜`3.2.6` のみ**（[トラッカー](db/CPL_Phase1_KPI_Tracker.md)）。`3.2.7`〜は Phase 2 または装備・構造クラスタ拡張。
...
> **`3.1.1_AviationLegal0.mdx` はリポジトリに存在**（[14](14_記事単元網羅とバックログ.md) と Supabase を正とする）。
...
#### 航空気象
> **注（2026-04-12）**: 以下 3 本は **スタブ MDX 済み**（本文化は Phase 2 で継続）。`3.3.12` は `learning_test_mapping` 追補済み（[14](14_記事単元網羅とバックログ.md)）。
...
- **07 との関係**: [07_PPL_Master_Syllabus.md](07_PPL_Master_Syllabus.md) に既に同トピックの `PPL-*` 記事がある場合は **新規 CPL 専用 ID で重複執筆しない**。既存記事を正本とし、`learning_test_mapping` のみ追加・更新する。
- **粒度**: 原則 **1 トピック 1 記事**（1 問 1 記事は避ける）。
- **手順の例**: [docs/db/APPLICABLE_EXAMS_PILOT.md](db/APPLICABLE_EXAMS_PILOT.md)（航空工学・空力の基礎理論パイロット）
...
2. ✅ `docs/db/CPL_Phase1_KPI_Tracker.md` - Phase 1 KPI（19本）進捗の正本
...
- [x] KPI 19 本の MDX 配置（`src/content/lessons` に全 ID あり — [トラッカー](db/CPL_Phase1_KPI_Tracker.md)）
- [ ] KPI 19 本の **本文化**（現在 **12/19** — トラッカー集計を更新しながら推進）
- [ ] `learning_test_mapping` の欠損埋め（例: `3.4.1`〜`3.4.3` — [14](14_記事単元網羅とバックログ.md)）
- [ ] 品質チェックとレビュー
...
- `docs/db/CPL_Phase1_KPI_Tracker.md` - **Phase 1 KPI（19本）進捗の正本**
...
**次のステップ**: [CPL_Phase1_KPI_Tracker.md](db/CPL_Phase1_KPI_Tracker.md) のスタブ 7 本を本文化し、Phase B の **5/19** を目指す
