...
**作成日**: 2025年1月16日
**最終更新**: 2026年4月12日（`3.3.*` スタブ配置・[14](14_記事単元網羅とバックログ.md) MCP 再集計に整合）
**バージョン**: v1.2.1
...
#### 航空法規（DB 4件目あり・リポジトリ MDX は3本 + 新規2記事）

> **整合（2026-04）**: Supabase `learning_contents` には **`3.1.1_AviationLegal0`** が公開済みだが、`src/content/lessons/3.1.1_AviationLegal0.mdx` は **無い**（要同期）。詳細は [14_記事単元網羅とバックログ.md](14_記事単元網羅とバックログ.md)。

1. ⚠️ 3.1.1_AviationLegal0 - 戦闘機乗りへの第一歩（DB あり・**MDX 要同期**）
