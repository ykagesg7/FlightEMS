...
- **NOAA Aviation Weather Center API**: 航空気象データAPI（認証不要・無料）
  - METAR（定時飛行場実況気象通報式）
  - TAF（飛行場予報）
- **RainViewer**: 計画タブ地図の降水レーダータイル（個人・教育向け無料API、[帰属表示必須](https://www.rainviewer.com/api.html)）。マニフェスト CORS 時は `VITE_RAINVIEWER_USE_MANIFEST_PROXY=true` で `/api/rainviewer-maps` を利用
- **Open-Meteo**: 気圧面の風など数値予報（非商用無料枠・[CC BY 4.0 帰属](https://open-meteo.com/en/licence)）。地図の上層風バーブ格子・計画タブの `useOpenMeteoWind` で利用（コール数・商用可否に注意）
...
```bash
# WeatherAPI.com（一般気象情報用・ブラウザ向け。任意）
VITE_WEATHER_API_KEY=your_weather_api_key

# サーバー側（Vercel / npm run dev の /api/weather プラグイン）。実データ取得に必須
...
#### **Vercel で API とフロントを一体起動する場合（任意）**

`npm run dev:full` は **`cross-env VITE_VERCEL_DEV_API_ORIGIN=http://localhost:3000 vercel dev --yes`** 相当です。Vercel が **`http://localhost:3000`** で API（`api/*`）を提供し、Vite は **`http://localhost:5173`** で動きます。ページを **5173 で開く場合**、ブラウザからの API は **3000 へ直接**送られます。**`localhost` と `127.0.0.1` は別オリジン**のため、フロントを `http://localhost:5173` で開いているのに API だけ `http://127.0.0.1:3000` だと CORS で失敗しうる。既定は `localhost` に揃え、`getDevApiBase` でも表示中ホストに合わせて補正する。**3000 の URL で開く**ことも可能です。`vercel dev` を別ポートにしたときは `VITE_VERCEL_DEV_API_ORIGIN` をそのオリジンに合わせてください。

クローン直後やチーム／プロジェクト変更後は **`npx vercel link`** が必要です（`.vercel` は Git 対象外）。

**`vercel dev` が失敗するとき**

| メッセージ | 意味と対処 |
|------------|------------|
| *Your Project was either deleted, transferred…* | リンク先プロジェクトが無効。プロジェクトルートの **`.vercel` フォルダを削除**し、`npx vercel login` のうえ **`npx vercel link`** でチームとプロジェクトを選び直す。 |
| *Project names must be lowercase… (400)* | 新規プロジェクト名に **カレントフォルダ名**（例: `FlightAcademyTsx` の大文字）が使われ、Vercel の命名規則に弾かれている。**既存の小文字プロジェクトを選択**するか、新規名を **`flight-planner`** のように **すべて小文字**で指定する（`package.json` の `name` と揃えるとよい）。 |

**このリポジトリが接続すべき Vercel 側（ダッシュボード／Vercel MCP で照合済み）**
...
```json
{"orgId":"team_jHZWbMZXYsgRZG2RulyP9jek","projectId":"prj_1GcGeUyndywgTEuAmQFAHbXLSqUz"}
```
...
**OpenSky（航空機レイヤー）**: フロント用の API キー設定は**不要**です。OpenSky Network の公開 API を**サーバ側プロキシ**（`api/opensky-states`）経由で呼びます。匿名利用でレート制限あり（429 時はクライアントが前回マーカーを Stale 表示で保持）。**任意・推奨**: `OPENSKY_CLIENT_ID` / `OPENSKY_CLIENT_SECRET`（OpenSky Account → API Client、OAuth2 client credentials）を Vercel / `.env.local` に設定するとクレジット枠が増え本番が安定しやすい（`.env.example` 参照。**2026-03〜 Basic `OPENSKY_USERNAME/PASSWORD` は非対応**）。クライアントは **3 分間隔**でポーリング（`TRAFFIC_POLL_MS`）、**パン・ズームでは再取得しない**。BBOX は **0.5° 量子化**（CDN キャッシュ共有）。プロキシ成功時 `Cache-Control: s-maxage=150, stale-while-revalidate=180`。上流タイムアウト: OAuth トークン **8s**・states **18s**（`api/_lib/openskyStatesCore.ts`・`api/_lib/openskyOAuthToken.ts`）。**Vercel 本番**（`process.env.VERCEL`）では upstream を **`fetch()` のみ**（IPv4 直結フォールバックは ETIMEDOUT / 30s 超過のため無効）。ローカル dev は fetch → hostname → IPv4 の順で試行。**`vercel.json` のトップレベル `regions: ["fra1"]`** でサーバレスを Frankfurt に寄せ、OpenSky（欧州）への往復を短くする。接続失敗時は **502/504**。`maxDuration` は **`api/**/*.ts` で 15s** と整合。ポップアップ HTML は `popups/openskyTrafficPopup.ts`（`autoPan: false`）。3 分更新で開いていたポップアップは `useLiveTrafficLayer` が icao を追跡し内容更新後に再 open する。

### **Cursor で Chrome DevTools MCP を使う（任意）**

AI から **実ブラウザ**（コンソール・ネットワーク・スナップショット等）を扱うには、`.cursor/mcp.json` に **`chrome-devtools`** サーバを定義する（[`.cursor/mcp.json.example`](../.cursor/mcp.json.example) 参照）。要件は Node **v20.19+** と **Chrome**。手順・オプトアウト・Slim モードは **[Cursor_MCP_Setup.md](Cursor_MCP_Setup.md)** の「Chrome DevTools MCP」節を参照。設定変更後は **Cursor の再起動** が必要。
...
#### **デュアルテーマアーキテクチャ**
- **MarketingLayout**: マーケティング用ダークテーマ（黒×黄色アクセント。ブランド移行は Phase C）
...
### **共通コンポーネントとページ固有コンポーネントの分離**

プロジェクトでは、コンポーネントを次の2カテゴリに分類して配置する（旧称「ハイブリッド方式」と同義）：
...
詳細は [Component_Structure_Guide.md](Component_Structure_Guide.md) を参照してください。

---

## 🔧 主要コンポーネント・フック

### **ギャラリー・ショップ（撤去済み・2026年4月）**

アプリ内ギャラリー・ショップ用のページ・フックは削除。`/gallery`・`/shop`・`/experience` のルートは **削除済み**（2026-04-12）。スキーマは [02_System_Spec.md](02_System_Spec.md)「DB スリム化」と `scripts/database/20260411_drop_gallery_shop_purchase.sql` を参照。
...
#### **主要フック**
- **useArticleProgress**: 学習進捗管理、記事読了時のXP付与（2025年1月実装、2026年1月5日改善）
  - `refreshProgress`: Supabaseから最新の進捗データを再取得する関数
  - ページ遷移後に自動的に最新データを取得
  - content_idベースで進捗を取得（slugとの不一致を解消）
- **useArticleStats**: 記事統計データ管理（いいね、コメント、閲覧数）
- **記事シリーズ**: MDX `meta.series` / `meta.order` は推奨読み順・ダッシュボードの「次に読む」用（閲覧ロックなし）

### **ゲーミフィケーションシステム（2025年1月実装）**

#### **主要コンポーネント**
- **RankBadge**: ランクバッジ表示（統合ランクシステム対応、PPL中間ランク含む）
...
#### **主要フック**
- **useGamification**: ランク・XP・ミッション管理（統合ランクシステム対応）
- **usePPLRanks**: PPLランク管理（中間ランクの取得・表示）
...
**PPL中間ランク（記事完了ベース）:**
- **PPL空力基礎初級** → **PPL空力基礎中級** → **PPL空力基礎マスター**
- **PPL性能基礎初級** → **PPL性能基礎中級** → **PPL性能基礎マスター**
- **PPL航空力学マスター** → **PPL航空工学マスター** → **PPL** (自家用操縦士、500XP到達)

**上位ランク（XPベース）:**
- **Wingman** (1,200 XP) → **CPL** (事業用操縦士、1,000XP到達) → **Ace** (1,500 XP)
- **Master** (2,000 XP) → **Legend** (2,500 XP)

**注意**: 削除されたランク（Spectator, Trainee, Student, Apprentice, Pilot）は既存ユーザーに対して`fan`に変換されました。

#### **ランク条件管理**
- 各ランクに複数の条件を設定可能（XP、記事読了数、試験パス、ミッション達成など。旧ショップ連動の購買条件は DB から削除済み）
...
- **Leafletの経度ラップ処理**: `MapContainer`に`worldCopyJump={true}`を設定
...
- `src/setupTests.ts`: テストセットアップ（Supabase/Leafletモック、環境変数モック）
  - テスト環境用のSupabase環境変数（`VITE_SUPABASE_URL`、`VITE_SUPABASE_ANON_KEY`）をモック
  - `src/utils/supabase.ts`ではテスト環境（`MODE === 'test'`）では環境変数チェックをスキップ
...
- **エンゲージメント追跡**: streak_records、user_achievements テーブル追加（purchase_history は後に未使用のため削除 — 2026年4月）
...
- **setupTests.tsに環境変数モック追加**: テスト環境用のSupabase環境変数をモック
- **未使用インポートの削除**: `useArticleProgress.ts`から未使用の`calculateTotalArticleXp`と`updateStreak`を削除
- **switch文のcaseブロック修正**: `ArticleDashboard.tsx`のswitch文でcaseブロックを`{}`で囲んでlintエラーを解消
- **定数条件の削除**: 不要な`${true ? ... : ...}`や`{false && ...}`を削除
- **useCallbackの依存配列修正**: `useArticleProgress.ts`の依存配列に不足していた依存関係を追加

### **2026年1月5日 - 記事ステータス判定と進捗更新ロジックの改善**
- **記事ステータス判定の修正**: `ArticleDashboard`でcontent_idベースの進捗取得に変更
  - `meta.slug`と`content_id`の不一致による進捗取得エラーを解消
  - Supabaseの最新データを反映したステータス表示
- **useArticleProgressにrefreshProgress関数を追加**: ページ遷移後に最新データを再取得
- **ページ遷移時の進捗保存改善**: `visibilitychange`イベントで確実に保存
  - 完了時（100%到達）は即座に保存（遅延なし）
  - ページが非表示になる前に進捗を保存
- **ArticleDashboardでの自動更新**: ページ遷移後に`refreshProgress`を実行
...
- PPL記事システム実装（15記事完了: Phase 1記事13件、Phase 2記事2件、進捗10.0%）
- 記事一覧・詳細の閲覧と `ReviewContentLink`（ゲスト登録 CTA）の動作確認
...
**最終更新**: 2026年4月25日（Node 18+ 表記。記事ステータス判定等の各節は従来どおり）
**バージョン**: Technical Development Guide v3.0
