...
- **目的**: エアロバティックチーム「Whisky Papa」のファンサイト兼パイロット学習プラットフォーム
...
4. **ゲーミフィケーション**: 統合ランクシステム（PPL中間ランク + XPベースランク: Fan → PPL中間ランク → PPL → Wingman → CPL → Ace → Master → Legend）、XP、ミッション、ストリーク、達成通知
...
### ファンサイト仕様強化（2026年2月1日）
- ✅ **Aboutページ**: パイロット紹介セクションをコメントアウト（本人許可取得後に復活予定）、MISSION BRIEFを拡張（サイト目的・メリット・注意事項を追加）
- ✅ **Shopページ**: 公式公認取得後に復活予定のため非表示（ナビ削除、/shopはホームへリダイレクト）
- ✅ **Scheduleページ**: Marketing Section・Footer Noteをコメントアウト、2026年空のイベントスケジュールを詳細化（確定4件+例年15件）

### ファンサイト統一対応（2026年1月6日）
- ✅ **WhiskyPapaファンサイトとして統一**: 公式サイトからファンサイトに変更
  - Header/Footerを「ファンサイト」表記に変更
  - Home/AboutページをWhiskyPapaファンサイト仕様に統一
  - Aboutページのメンバー紹介（THE CREW）セクションをコメントアウト（復活可能）
- ✅ **コードの簡素化**: WhiskyPapa/FlightAcademyの切り替え機能を削除
...
**バージョン**: Documentation Index v3.6
