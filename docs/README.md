...
npm run test:e2e      # Playwright（初回: npx playwright install chromium。webServer が `VITE_UNLOCK_ALL_SERIES_ARTICLES=true` で build し CPL スタブ記事を開くテスト含む。CI 時は既存 preview を再利用しない）
npm run dev:unlock-series  # 手動検証: シリーズ順次ロックを無効化した Vite dev
