...
- **2026-08-17（週次テレメトリ ISO 火曜）**: レビューを **火曜 09:00 JST**（月〜日）へ切替。Slack `#fa-telemetry` の Facts は日本語（`@` なし）。フェーズ2b は Skill `weekly-telemetry-review` で正本 PR。フェーズ2c L0 は `APPROVE-DOC`。初回新窓 W34 は 8/25。正本 [ops/Weekly_Telemetry_Review.md](ops/Weekly_Telemetry_Review.md)。
