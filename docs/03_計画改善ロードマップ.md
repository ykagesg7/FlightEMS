...
**最終更新**: 2026年4月12日
**バージョン**: Roadmap v4.0.2（Strategy v1.2・CPL 主軸・独立運営）
...
- [ ] **GA4**: Vercel 本番に `VITE_GA_MEASUREMENT_ID`（`G-XXXXXXXXXX`）を設定し、デプロイ後に [リアルタイムレポート](https://support.google.com/analytics/answer/9271392)で `page_view` がルート遷移ごとに増えることを確認（Network で `google-analytics.com/g/collect` または `googletagmanager.com`）
