...
**最終更新**: 2026年4月12日
**バージョン**: Roadmap v4.0.3（Strategy v1.2・CPL 主軸・独立運営）
...
- [ ] **GA4**: Vercel 本番に `VITE_GA_MEASUREMENT_ID`（`G-XXXXXXXXXX`）を設定し、**Production 対象・再デプロイ後**に [リアルタイム](https://support.google.com/analytics/answer/9271392)で `page_view` を確認。データがゼロのときは (1) 変数名の typo (2) Production 以外にだけ設定していないか (3) 広告ブロッカー無効で Network に `google-analytics.com/g/collect` 等が出るか (4) ビルド成果物の JS に `G-` ID が文字列として含まれるか（`vite.config.ts` の `define` で `process.env` から埋め込み）を確認
