...
export const QuestionComponent: React.FC<QuestionComponentProps> = ({
                ...
                <label
                  ...
                  className={`block p-3 rounded-xl border transition-all duration-200 cursor-pointer text-base font-medium
                    ${currentAnswer === optionId
                      ? 'bg-sky-500 border-sky-400 ring-2 ring-sky-300 text-white'
                      : theme === 'dark'
                        ? 'bg-slate-700 border-slate-600 hover:bg-slate-600 text-slate-100'
                        : 'bg-white border-gray-300 hover:bg-blue-50 hover:border-blue-300 text-gray-900'}
      ...
      case QuestionType.NUMBER_INPUT:
        return (
          <input
            ...
            className={`w-full p-3 border rounded-xl focus:ring-2 focus:ring-sky-500 focus:border-sky-500 outline-none transition-all duration-200 ${theme === 'dark'
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      <p id={questionTextId} className={`whitespace-pre-line leading-relaxed mb-3 ${theme === 'dark' ? 'text-slate-200' : 'text-slate-800'
        }`}>{question.text}</p>
      {question.imagePlaceholder && (
        <div className={`my-3 p-2 rounded text-center ${theme === 'dark' ? 'bg-slate-700/50' : 'bg-gray-100'
          }`}>
          <p className={`text-sm italic ${theme === 'dark' ? 'text-slate-400' : 'text-gray-600'
      ...
      {feedback && (
        ...
        >
          <p className={`font-semibold ${feedback.isCorrect
          ...
          <p className={`mt-2 whitespace-pre-line leading-relaxed ${theme === 'dark' ? 'text-slate-300' : 'text-slate-700'
      ...
      {showAnswer && question.type === QuestionType.TEXT_INPUT && !feedback?.isCorrect && (
        <div className={`mt-2 p-3 rounded-xl ${theme === 'dark' ? 'bg-slate-700' : 'bg-gray-100'
          }`}>
          <p className={`text-xs mb-1 ${theme === 'dark' ? 'text-slate-400' : 'text-gray-600'
            }`}>模範解答:</p>
