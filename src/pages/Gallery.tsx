...
import AdminReviewPanel from '../components/gallery/AdminReviewPanel';
import AdminEventPanel from '../components/gallery/AdminEventPanel';
...
const Gallery: React.FC = () => {
  ...
  const handleDeleteMyPhoto = async (photoId: string) => {
    ...
    if (!photo) return;
    const ok = window.confirm('この写真を削除しますか？（いいねも失われます）');
    if (!ok) return;
