...
**作成日**: 2025年1月16日
**最終更新**: 2026年4月（14 網羅ドキュメントとの整合: 気象 3.3.*・法規 3.1.1 の実態注記）
**バージョン**: v1.2
...
#### 航空気象（ハブ＋スタブのみ・粒度記事は未作成 + 新規9記事）

> **整合（2026-04）**: `src/content/lessons` に **`3.3.*` 系 MDX は未配置**。入口は `weather_basics` と `CPL-Hub-Meteorology`。網羅状況の一覧は [14_記事単元網羅とバックログ.md](14_記事単元網羅とバックログ.md)。

1. 🆕 3.3.1_StandardAtmosphere - 標準大気と大気構造（計画・MDX 未作成）
2. 🆕 3.3.2_CloudsAndPrecipitation - 雲と降水のメカニズム（計画・MDX 未作成）
