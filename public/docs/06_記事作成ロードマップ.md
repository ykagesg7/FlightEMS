...
> **整合（2026-04-12）**: `3.3.1`〜`3.3.12` の **スタブ MDX は `src/content/lessons` に配置済み**。`learning_test_mapping` は **10/12 本**（`3.3.8`・`3.3.12` は未マッピング）。入口は引き続き `weather_basics` と `CPL-Hub-Meteorology`。数値の正本は [14_記事単元網羅とバックログ.md](14_記事単元網羅とバックログ.md)。
...
#### 航空気象
> **注（2026-04-12）**: 以下 3 本は **スタブ MDX 済み**（本文化は Phase 2 で継続）。`3.3.12` は `learning_test_mapping` 未整備（[14 §2](14_記事単元網羅とバックログ.md)）。
