...
| **2026-W28**（〜2026-07-15） | **6** | ① **Tier A**: 着氷/着氷の防止 **4 問** → `engineering_basics`（[`20260706_learning_test_mapping_icing_prevention_w28.sql`](../scripts/database/20260706_learning_test_mapping_icing_prevention_w28.sql) MCP 済）。② **PPL-3-1-1**・**PPL-3-1-2** 深文化 + `learning_contents` **301–302** + mapping（[`20260707_*`](../scripts/database/INDEX.md)）。③ 法規 Callout **`3.1.4`/`3.1.5`** ← PPL-5-*。④ Subject 3/4 **スタブ 15 本** + Gemini 骨子。⑤ MCP 再集計 — 未マッピング **17**、mapping **111 行**、`learning_contents` **99**。 |
