...
| **`learning_test_mapping` がある記事** | **16** | `learning_content_id` 単位（2026-04-10 時点） |
| **リポジトリ `src/content/lessons/*.mdx`** | **66** 前後 | `docs/06` Phase1+2 相当の CPL スタブ 35 本を 2026-04-10 に追加（再集計で更新） |
...
## 2. `learning_test_mapping` が付いている記事（16件）

クイズ結果の **ReviewContentLink** がマッピング優先で参照しうる `learning_content_id`:

| id | 備考 |
|----|------|
| `3.1.1_AviationLegal0` | DB・マッピングあり。**MDX スタブあり**（2026-04-10） |
| `3.1.2_AviationLegal1` | MDX あり |
| `3.1.3_AviationLegal2` | MDX あり |
| `3.1.4_AviationLegal3` | MDX あり |
| `3.2.1_PropellerTheory` 〜 `3.2.6_InstrumentSystem` | 各 MDX あり |
| `CPL-Hub-Communication` | ハブ |
| `CPL-Hub-Meteorology` | ハブ |
| `CPL-Hub-Navigation` | ハブ |
| `engineering_basics` | 工学入口 |
| `weather_basics` | 気象スタブ |
| `PPL-1-1-1_TemperatureBasics` | PPL からのマッピング例 |

他の PPL 記事（`PPL-1-1-2` 以降など）にも **個別マッピングを増やす余地**がある（現状は上記以外はマッピング行なし、または別テーブル運用の要確認）。

---

## 3. リポジトリに **ある** MDX（`src/content/lessons`・66 前後）
...
| **粒度のある気象記事 `3.3.*`** | **2026-04-10** に `3.3.1`〜`3.3.12` スタブ MDX を追加。ハブ `CPL-Hub-Meteorology` と併用。 |
...
テンプレート: [docs/templates/CPL_Lesson_Stub_Template.mdx](../docs/templates/CPL_Lesson_Stub_Template.mdx)
