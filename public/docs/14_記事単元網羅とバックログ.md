...
**作成日**: 2026-04-10  
**スナップショット更新**: 2026-04-12（マッピング 3.3.8 / 3.3.12 追補後・MCP 再取得）  
...
| **`learning_test_mapping` がある記事** | **44** | `learning_content_id` 単位・行数とも 44（2026-04-12 追補後） |
...
## 2. `learning_test_mapping` が付いている記事（44件）

クイズ結果の **ReviewContentLink** がマッピング優先で参照しうる `learning_content_id`（2026-04-12 追補後・`ORDER BY learning_content_id`）:
